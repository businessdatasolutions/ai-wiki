...
**Venue (final):** ACM Transactions on Information Systems (TOIS), 2025
**DOI (final):** https://doi.org/10.1145/3748302
**arXiv:** 2404.13501 (**v1, 21 Apr 2024 — the version held here**)
**URL:** https://arxiv.org/abs/2404.13501
**OpenAlex:** W4412203333
**Companion repo:** https://github.com/nuster1128/LLM_Agent_Memory_Survey
...
## Identity note (pre-flight Check 2)

The citable record is the **2025 ACM TOIS article**; the full text held here is the
**2024 arXiv v1 preprint**, which is a year older and not the camera-ready. The TOIS version
is paywalled and was not retrievable. Cite the DOI, but scope any claim to what the preprint
actually says — section numbering and the author-order differ between the two.

## Scope and conversion note

Full preprint, all 39 pages. `pdftotext -layout` flattens the taxonomy tables, which carry a
lot of this survey's content — verify against the PDF before quoting.

**Why acquired.** Backing for `CLAUDE.md` §Retention and §Tier vocabulary, both of which
currently assert a memory model without citing one. Covers the write/read/reflect loop,
memory duplication and conflict handling, and evaluation of memory modules.
