...
isbn: ''
url: http://arxiv.org/abs/2607.13285
citekey: ''
abstract: The capability of a modern AI agent depends not only on its foundation model but also on its
  harness, which constructs prompts, manages state, invokes tools, and coordinates execution. As models,
  APIs, environments, and requirements evolve, the harness must be continually modified. Before such a
  change can be made, a developer or coding agent must identify all code locations that implement the
  target behavior. This is difficult because production harnesses are large, tightly coupled, and behaviorally
  distributed, while modification requests describe what the system should do and repositories are organized
  by files and modules. Code search, repository indexing, and long-context processing ease inspection,
  but still leave this behavior-to-code mapping to be recovered by hand. Behavior localization is therefore
  a central bottleneck in harness evolution. We introduce the Harness Handbook, a behavior-centric representation
  synthesized automatically from a harness codebase via static analysis and LLM-assisted structuring,
  linking each behavior to its corresponding source. We also introduce Behavior-Guided Progressive Disclosure
  (BGPD), which guides agents from high-level behaviors to relevant implementation details and verifies
  candidate locations against the current source. On diverse modification requests from two open-source
  harnesses, Handbook-Assisted planning improves behavior localization and edit-plan quality while using
  fewer planner tokens, with the largest gains on scattered sites, rarely executed paths, and cross-module
  interactions. Evolving complex agentic systems thus depends not only on generating edits, but also on
  determining where those edits should be made.
attachment: harness-handbook-making-evolving-agent-harnesses-readablenavigable-and-editable.pdf
...
---
                                             ...
                                             Ruhan Wang⋆1,2 , Yucheng Shi† 1 , Zongxia Li1,3 , Zhongzhi Li1,4 , Yue Yu2 , Junyao Yang1,5 , Kishan
                                             Panaganti1 , Haitao Mi1 , Dongruo Zhou2 , Leoweiliang1
                                             1
                                              Tencent HY LLM Frontier 2 Indiana University 3 University of Maryland, College Park 4 University of
                                             Georgia 5 National University of Singapore
                                             ⋆
...
arXiv:2607.13285v1 [cs.AI] 14 Jul 2026
                                             ...
                                             We introduce the Harness Handbook, a behavior-centric representation synthesized automatically
                                             ...
                                             corresponding source code. We also introduce Behavior-Guided Progressive Disclosure (BGPD),
...
We refer to identifying all implementation sites associated with a behavioral request as behavior localization.
...
address.
We address this gap with Harness Handbook, an operational behavior representation that connects system
...
explicit can make harness evolution more accurate and efficient. Our contributions are as follows:
   • We define behavior localization as finding all code locations that implement the behavior described in a
     modification request. This step is necessary before a complete edit can be planned.
   • We introduce Harness Handbook, which organizes implementation knowledge by what the harness
   ...
   • We introduce Behavior-Guided Progressive Disclosure (BGPD), a workflow that guides coding agents
     from high-level behavior descriptions to relevant implementation details in stages.
   • We evaluate Harness Handbook on diverse modification requests from two open-source harnesses. We
...
execution logic as implementation details, Code as Agent Harness formalizes the harness as an executable and
...
3     Harness Handbook
Harness Handbook has three parts for understanding and modifying an agent harness. The representation
organizes source around runtime behavior (Section 3.1). The construction pipeline builds this representation
from a repository (Section 3.2). The modification workflow uses the handbook to guide code changes and
automatically resynchronizes it after every non-empty repository diff (Section 3.3).


                                                       3
 Harness Handbook                                                                                Progressive Disclosure

 1.System Overview          2.4 Main Loop                                                                  L1                                   L2                                         L3
   1.1 Architecture         The core orchestrator that iteratively performs reasoning, tool            Level 1:                                Level 2:                                   Level 3:
                            use, and observation until the exit conditions are met
   1.2 Execution Workflow
                                                                                                       System Overview                         Component Overview                         Unit Deep Dive
   1.3 Design Principles            Purpose
                                    Drive the agent to iteratively complete the task
                                                                                                                                                                                           1
 2. Execution Stages                Triggering Conditions                                                                                                                                  2

                                                                                                                                                     Stage            Component            3
   2.1 Initialization               Enter when loop starts or after each observation
                                                                                                                                                                                           4

   2.2 Planning                     Input                                                                                                                                                  5
                                                                                                                                                                                           6
                                    Current state, observation, memory, tool results
   2.3 Tool Execution                                                                                                                                Data            External Interface    7
                                                                                                                                                                                           9
                                    Processing Steps
   2.4 Main Loop (Repeat)
                                    Reasoning, tool selection, action execution, observation
                                                                                                       Overall description of the              Overview of all working units              Detailed analysis of each
   2.5 Observation
                                    Output                                                             entire harness system,                  that constitute the harness,               unit's internal logic, state
   2.6 Termination (Exit)           Updated state and observation for next iteration                   including architecture,                 including their roles, inputs/             transitions, edge cases,
                                    Related States                                                     execution model, major                  outputs, and interactions.                 and implementation.
 3. State and Data                  Messages, scratchpad, tool results, loop counter
                                                                                                       stages, and system goals.
   3.1 Key States
                                    Exceptional Cases
   3.2 Data Flow                    Tool failure, parse error, max steps reached, timeout
                                                                                                       Information Included                    Information Included                       Information Included
                                                                                                            System architecture                   Component list & taxonomy                    Detailed execution logic
 4. Components                      Key Functions
                                    _run_agent_loop(), _step(), _should_exit()                              Execution model                       Responsibilities & goals                     State transitions
   4.1 Prompting                                                                                            Stage overview                        Inputs / outputs                             Exception handling
                                    Configurations
                                                                                                            Data & state flow                     Dependencies & interactions                  Implementation details
   4.2 Memory                       Max steps, retry policy, exit criteria
                                                                                                            Design principles                     Key states involved                          Source code & snippets
   4.3 Tools                        Examples
                                                                                                            Entry and exit conditions                                                          Logs & examples
                                    Typical execution trace and logs
   4.4 Others
                                    Notes
                                    Loop termination depends on multiple exit conditions
                                                                                                       Shallow(High-level)                                                                      Deep (Implementation)



         Benefits of             Reduce Cognitive Load                       Improve Efficiency                   Support Multiple Roles             Enhance Maintainability                   Boost Learning
                                 Access the right information                Quickly locate relevant              Help developers, analysts,         Keep knowledge consistent
         Harness Handbook                                                                                                                                                                      Facilitate onboarding and
                                                                                                                  and agents understand
                                 at the right level.                         components and logic                                                    and up-to-date.                           knowledge transfer.
                                                                                                                  the system.




Figure 1 Overview of the Harness Handbook representation. Its three-level hierarchy progresses from a system-level
...
document tree D and a complementary state-register view Z.
A reader normally starts at L1 (system overview), which summarizes the architecture, execution model, major
stages, and global data flow. The reader then moves to L2 (component overview) for the responsibilities,
inputs, outputs, dependencies, and local state of a selected stage. Finally, L3 (unit deep dive) links that
stage to source-grounded implementation entries. The complementary view Z records state relationships that
cross stage boundaries.
Two rules keep the representation useful. Progressive disclosure means that readers move from L1 to L3 only
when a task requires more detail. Behavior–implementation alignment means that every active L3 locator
...
Construction builds a handbook from a repository R. The leaf mode g ∈ {function, file}, which remains fixed
for the lifetime of the handbook, determines the granularity of L3 entries: in function-as-leaf mode each L3
entry covers a whole function or one or more contiguous regions, whereas in file-as-leaf mode each L3 entry
represents a file.
The two modes differ in how the stage skeleton is obtained. function-as-leaf starts from a seed skeleton S0
...
reflects the harness’s execution stages is available and function-level L3 entries fit the available budget.
                                                                                                       ...
                                                                                                       4
              Phase I Static Fact Extraction                                         Phase II Behavioral Organization                                    Phase III Handbook Synthesis

 Source Repository    Static Analysis      Program Facts           Execution Skeleton Propose-Review Mapping Loop         Stable Mapping
                                                                                                                                                                         Tier 1 Overview
                                                                           Init                    Proposer                   Initz                                        Architecture Overview
    agent-harness/
                                                                                             Propose Assignment                                                            Execution Model
      planner.py                                                          Setup              (Functions → Stages)            Setup                                         Stage Relationships
    executor.py                                                                                                                                                            Global State Flow
                     Functions / Methods    Functions Table               Loop                                               Loop
                                                                                       Proposals        Reject & Refine
      memory.py
                     Call Graph             Call Graph
                                                                         Tool Call                                          Tool Call                                    Tier 3 Unit Cards
    tools/                                                                                         Reviewer                                Tier 2 Stage Cards
                     State Read /Write      State Accesses
                                                                                              Review & Feedback                             Execution Stage Overview       Behavior
                                                                          Parse                                              Parse
      parser.py      Classes /Modules       Unresolved Calls Log                                                                                                           Inputs / Outputs
                                                                                                                                            Purpose & Responsibilities

                     External Calls                                       Finish                                             Finish         Execution Flow                 State
                                                                                                              Accept
                                                                                                                                            Internal Units                 Code Anchors


                       Output: Program Graph                                               Output: Behavioral Mapping                                            Output: Handbook




Figure 2 Construction pipeline for Harness Handbook. Static analysis extracts source-linked facts, behavioral
organization maps source units to execution stages, and hierarchical synthesis builds the L1–L3 handbook.

file-as-leaf instead infers the stage skeleton rather than starting from a seed; we use it when no such seed is
...
named external boundaries, source locations, signatures, and call edges. The program graph G keeps only calls
...
Phase II. Behavioral Organization. The pipeline organizes source units into the execution-stage skeleton S
...
handbook traceable to the source. Finally, the pipeline renders V and packages the structured data needed for
...
request q, the workflow uses the handbook H together with its corresponding repository R. The leaf mode g
...
process produces a set of source-grounded evidence for planning. Second, the planner converts the evidence
                                                                                                      ...
                                                                                                      5
into an edit plan P and action declarations Γ. Third, a separate executor applies P to the repository. Finally,
any non-empty diff triggers handbook resynchronization. Algorithm 1 summarizes the complete workflow.

Algorithm 1 Handbook-Guided Modification with Automatic Resynchronization
Require: request q, handbook package H, corresponding repository snapshot R
Ensure: plan P and declarations Γ; updated repository R′ , diff ∆, and handbook package H′
 1: g ← LeafMode(H)
 2: Ebq ← BGPD(q, H, R)
 3: (P, Γ) ← Plan(q, Ebq )
 4: R′ ← Execute(R, P)
...
Localization begins at the level of execution stages. BGPD first uses L1 and L2 of D to identify the stages
directly relevant to the request. It then follows Z to include stages coupled through shared state. This step
...
BGPD then expands the candidate set along call relations. In function-as-leaf mode, it uses the function-call
graph; in file-as-leaf mode, it uses the induced file-call graph. External boundary nodes provide context but
are never returned as edit sites.
Up to this point, BGPD operates on the handbook. It then opens the current repository R, resolves the
candidate locators, and retains only the sites that remain relevant to q. This yields the verified evidence Ebq .
...
The planner converts Ebq into an edit plan P. Each edit block specifies where the change should be made,
...
and one of three action types: modify, add, or remove. The resulting declarations are collected as Γ =
(Γmodify , Γadd , Γremove ). A rename is represented as one removal and one addition.
The executor applies P to R and produces the updated repository R′ . The resulting diff is ∆ = Diff(R, R′ ).
Because ∆ is the factual record of what changed, it drives handbook invalidation. By contrast, Γ is used only
...
identify added, removed, and modified units: in function-as-leaf mode, it matches functions using fingerprints


                                                            6
that do not depend on line numbers, and in file-as-leaf mode, it matches files using file-set differences and
content hashes.
Second, it determines the update scope. If the stage skeleton S remains valid, only the affected entries and
their enclosing structure are refreshed; otherwise, Algorithm 2 is rerun on R′ with the same leaf mode g and
stored configuration Θ.
...
Given a natural-language request, a read-only planner built with NexAU [31] and powered by DeepSeek-V4-
Pro produces an edit plan P, and we score the quality of P directly.
We compare two arms: the Baseline explores the repository directly, whereas the Handbook-Assisted arm
...
Plan quality is judged independently by three models: GPT-5.5, Opus 4.8, and DeepSeek-V4-Pro.

Benchmarks and Modification Requests. We evaluate Terminus-2 [32] and Codex [25], two open-source
agent harnesses that adopt the two leaf modes from Section 3.2: Terminus-2 uses function-as-leaf because it
has a reliable seed skeleton, and Codex uses file-as-leaf to infer the hierarchy at repository scale.
Each harness contributes 30 behavior-driven modification requests. They are grouped by type into Query (Q),
which modifies existing behavior without revealing target locations; Cross-file (CF), which adds an end-to-end
capability spanning files or modules; and Search-Hostile (SH), which places relevant implementations in
locations that are hard to recover by keyword search. Requests are also labeled Easy, Medium, or Hard by
...
sites are accurate), Scope Control (whether the plan stays focused, with less scope bloat), and Reasoning
(whether the rationale and supporting evidence are adequate). The weighted score is

                                   S = 0.5SLoc + 0.25SScope + 0.25SReason ,                                  (1)
...
Win rate (%)                                        Win rate (%)
               ...
               30                                                  32.5                                                                                                                                  -8.6%
                                                                   ...
                                                                   25.0                                      20.0                                                           0.04
               20
                          Codex         Terminus                          GPT-     Opus     DeepSeek                  GPT-      Opus     DeepSeek                                   Codex             Terminus
                                                                          5.5       4.8      V4-Pro                   5.5        4.8      V4-Pro
               (a) Overall Win Rate                                                        (b) Per-Judge Win Rate                                                                    (c) Token Cost

Figure 3 Plan quality and planner token usage on Codex and Terminus-2. (a) Overall win rates aggregated across the
...
planner tokens per request; lower values indicate greater efficiency.
                                                                                     ...
                                                                                     Baseline        Handbook-Assisted
                                           Localization                                                 Scope Control                                                              Reasoning
                                                                                    40.0                                                 40.0       +13.4                          +3.4
                      40.0                  +10.0                                                           -3.4                                                                                          +3.3

               Codex
                                                                   +6.6    +2.2     32.5      +3.3                              +1.1     32.5
                      32.5        -10.0                                                                               +3.4                                                                     -6.6
...
Win rate (%)
               ...
               Terminus
                          ...
                          20
                                                                                    10.0
                          10                                                                                                               10
                                 GPT-      Opus      DeepSeek             Avg.              GPT-        Opus       DeepSeek    Avg.             GPT-                           Opus        DeepSeek      Avg.
                                 5.5        4.8       V4-Pro                                5.5          4.8        V4-Pro                      5.5                             4.8         V4-Pro



Figure 4 Per-judge win rates for three evaluation dimensions. Rows show results for Codex and Terminus-2, while
...
the winner when their scores differ by at least δ = 3 points; otherwise the comparison is a tie. The win rate
...
does not depend on any single judge.
                                                                                                        ...
                                                                                                        8
                                                                    Baseline         Handbook-Assisted
                             Codex                               Terminus                                Codex                             Terminus
                              +16.3                60                           +33.3            +33.3                        50
               55                                                                        50.0                                                 +18.0   +3.7
                                                                                                                    +11.2
...
Win rate (%)
               ...
               15                                  20                                    15.0                                 20
                    Q         CF      SH                  Q         CF          SH              Easy     Medium Hard               Easy    Medium Hard
                            (a) Modification Request Type                                                 (b) Localization Difficulty

Figure 5 Win rates by (a) modification request type and (b) localization difficulty. Q, CF, and SH denote Query,
...
valid requests; missing outputs and planner errors are excluded. Wrong indicates zero overlap with the reference plan
...
[31] Nex-AGI Team, Yuxuan Cai, Lu Chen, Qiaoling Chen, Yuyang Ding, Liwen Fan, Wenjie Fu, Yufei Gao, Honglin
     ...
     construction. arXiv preprint arXiv:2512.04987, 2025. URL https://arxiv.org/abs/2512.04987.
[32] Mike A. Merrill, Alexander G. Shaw, Nicholas Carlini, Boxuan Li, Harsh Raj, Ivan Bercovich, Lin Shi, Jeong Yeon
     ...
     command line interfaces. In The Fourteenth International Conference on Learning Representations, 2026. URL
...
[35] Vadim Lomshakov, Andrey Podivilov, Sergey Savin, Oleg Baryshnikov, Alena Lisevych, and Sergey Nikolenko.
     Proconsul: Project context for code summarization with llms. In Proceedings of the 2024 Conference on Empirical
...
This appendix expands the three construction phases in Section 3.2. The selected leaf mode g and construction
configuration Θ stay fixed through construction and later resynchronization. Both modes work from the same
static facts but organize them differently: function-as-leaf starts from a trusted seed skeleton S0 and places
functions within it, whereas file-as-leaf describes files first and infers the skeleton. Both ultimately produce
...
Algorithm 2 Harness Handbook Construction
Require: repository R; leaf mode g ∈ {function, file}; construction configuration Θ; seed skeleton S0 if g = function
    (otherwise ∅)
Ensure: handbook H containing rendered view V, document tree D, register view Z, fixed leaf mode g, and synchronization
    state Kg
    Phase I: shared static facts.
 1: G ← BuildGraph(R)                                                                          # deterministic; no LLM

    Phase II: mode-specific behavioral organization.
 2: if g = function then
 3:     (S, Ug ) ← OrganizeFunctions(R, G, S0 , Θ)                              # budgeted stage and membership review
 4:     Ug ← FinalizeFunctions(R, G, S, Ug )                                 # regions, ordering, and unmapped functions
 5: else                                                                                                        # g = file
 6:     (S, Ug ) ← OrganizeFiles(R, G, Θ)                                       # cards, stage inference, and assignments
 7:     if RefinementEnabled(Θ) then
 8:         (S, Ug ) ← RefineFileOrganization(R, G, S, Ug , Θ)
 9:     end if
10:     Ug ← FinalizeFiles(G, S, Ug )                                                  # within-stage order and coverage
11: end if

    Phase III: synthesis, grounding, and packaging.
12: (D, Z, B) ← Synthesize(g, R, G, S, Ug , Θ)
13: Z ← ValidateRegisterRefs(Z, S)
14: D ← ValidateAndFreezeLocators(D, R, g)
15: Kg ← Archive(g, G, S0 , S, Ug , B, Θ)                                            # S0 is stored only in function mode
16: H ← RenderAndPackage(D, Z, g, Kg )
17: return H



A.1     Phase I: Shared Static Fact Extraction
Phase I is a single deterministic analysis shared by both modes. Language-specific adapters parse R and build
the program graph G. An internal-function node records a function’s qualified name, source file, signature,
...
Phase II organizes the facts in G around execution behavior. Its output is the current stage skeleton S together
with the mode-specific organization state Ug . The two leaf modes differ in both their starting point and their
unit of organization.

A.2.1    Function-as-Leaf
Initial assignment. The function-as-leaf branch (g = function) starts from the seed skeleton S0 , whose
stage and state-register declarations initialize S. On the first pass, it considers every analyzable internal
function—that is, every non-synthetic function with usable source locations—and attempts to assign it to
one or more stages. The decision uses the function’s source in R, its callers and callees in G, the current
...
not reached, the process stops at the organization budget stored in Θ and keeps the remaining gaps explicit.
...
the enclosed source is hashed for later comparison. A boundary that cannot be aligned is marked needs-review ;
...
status, within-stage order, and record of unmapped functions constitute Ufunction .

A.2.2   File-as-Leaf
The file-as-leaf branch (g = file) uses no seed skeleton (S0 = ∅). It first describes each file, then infers S from
...
marked undescribed in the coverage record Y, so no scanned file is silently dropped.
...
unknown stages remain visible in Y rather than being forced into the nearest stage.
The organization variant stored in Θ determines whether the initial result is refined. oneshot, the default,
...
the organization budget is reached. Any files that remain unassigned stay visible in Y.
...
Together, the file cards, stage assignments, within-stage grouping and order, and coverage record Y constitute
Ufile .

A.3    Phase III: Hierarchical Synthesis and Packaging
Phase III turns (S, Ug ) into the document tree D and state-register view Z, records reusable generation
outputs in the cache B, and validates both against the repository before packaging.
...
In function-as-leaf mode, each L1, L2, and L3 node uses a bounded generate–review–revise process. The
...
In file-as-leaf mode, file cards are rendered directly. Each stage and system summary is generated once, with
a deterministic fallback if generation fails, and each parent summary waits for its child summaries.
The resulting descriptions and summaries are retained in B so that unchanged content can be reused during
later generation or resynchronization.

Building the state-register view. In function-as-leaf mode, the register identities and stage relationships in Z
come from the declarations in the current stage skeleton; generation turns these declarations into reader-facing
descriptions without inventing new registers. In file-as-leaf mode, the pipeline instead proposes cross-stage
...
being assigned by guesswork.
                                                       ...
                                                       16
Before rendering, the pipeline resolves every L3 locator in D against R and compares the located source with
...
Rendering and saved synchronization state. After validation, the pipeline renders D and Z as the reader-
facing view V. It then packages this view with D, Z, the fixed g, and the machine-readable resynchronization
state Kg as the handbook H. Across both modes, Kg retains the fixed leaf mode, program graph, current
stage skeleton, mode-specific organization state, construction configuration, and generation cache. The stored
configuration Θ records the selected organization or refinement variant, card-detail level, and the budgets
for organization, generation, and register extraction where applicable. In function mode, Kg also retains the
original seed skeleton S0 ; no seed is stored in file mode. Saving H in this form allows a later code change to
...
in Appendix A. It follows a request q through BGPD localization, edit planning, execution, and automatic
resynchronization. Throughout the workflow, the handbook H is paired with its matching repository R, and
the stored leaf mode g remains fixed.
...
Stage selection. Navigation begins at L1 and L2 of the document tree D stored in H. The agent reads the
system overview and stage index to select the execution stages whose behavior matches q. It then follows
the state-register view Z to add stages coupled through shared state. This step recovers behavior that is
...
Call-relation expansion. The candidate set is expanded along call relations in the program graph G retained
by the synchronization state Kg . In function-as-leaf mode, expansion follows the function-call graph; in
file-as-leaf mode, it follows the induced file-call graph. Named external boundary nodes may supply context
but are never returned as edit sites.

Source verification. All preceding steps operate on H. The agent then opens R, resolves each candidate
locator, and keeps only the sites that remain relevant to q. The result is the verified evidence Ebq . Each record
...
B.2    Edit Planning
The planner converts Ebq into an edit plan P whose blocks name a target file, an optional anchor, the supporting
...
collected as Γ = (Γmodify , Γadd , Γremove ), with a rename encoded as one removal and one addition.


                                                       17
B.3     Execution
Execution is delegated to a separate agent that applies P to R and produces R′ . The factual record of the
change is ∆ = Diff(R, R′ ).
...
syntax gate when one is configured. The diff ∆ drives handbook invalidation, whereas Γ is used only to check
...
whenever possible. If ∆ = ∅, resynchronization is skipped and H′ = H.

Version alignment. The procedure reparses R′ and builds the updated program graph G ′ . It compares G ′
with the stored graph G, using ∆ to identify added, removed, and modified units.
In function-as-leaf mode, functions are matched using body fingerprints that ignore line numbers. A function
that only moves is therefore treated as unchanged, and its locator is shifted by the corresponding line offset.
A rename is detected by matching the body below the signature line. In file-as-leaf mode, file-set differences
...
Before the observed actions are compared with Γ, a detected function rename is normalized to one removal
and one addition. This audit does not alter the ∆-driven invalidation scope.

Scoped update. Resynchronization next checks whether the current stage skeleton S can accommodate the
change. If it remains valid, S ′ = S and the affected part of Ug is updated to Ug′ . In function-as-leaf mode,
...
file-as-leaf mode, it removes obsolete cards and assignments, refreshes cards for new or changed files, assigns
new files, reorganizes affected stages, and updates the coverage record Y to Y ′ .
The pipeline then regenerates the affected L3 entries, their enclosing L2/L1 ancestors, and dependent state
registers. This produces the updated document tree D′ and state-register view Z ′ . Unaffected generation
outputs are reused from B. The updated cache B ′ retains these reusable entries and adds or replaces the
refreshed outputs.
If S can no longer accommodate the change, Algorithm 2 is rerun on R′ with the same leaf mode g and stored
configuration Θ. The function-as-leaf branch also reuses the stored seed skeleton S0 . The file-as-leaf branch
instead infers a new stage skeleton from the updated repository. During the full rebuild, Phase I produces
G ′ , Phase II produces (S ′ , Ug′ ), and Phase III synthesis produces (D′ , Z ′ , B ′ ). These components follow the
...
locator or source hash cannot be revalidated is marked frozen and excluded from localization. In function-as-leaf
mode, new or changed functions that cannot be mapped remain explicitly recorded as unmapped in Ufunction  ′
                                                                                                                 .
If a changed function previously had an active L3 entry, that entry is frozen until classification succeeds. In
file-as-leaf mode, files without a valid card or assignment remain in the coverage record Y ′ within Ufile
                                                                                                        ′
                                                                                                            .
...
validates the locators in D′ against R′ and the stage references in Z ′ against the updated skeleton, archives
G ′ and (S ′ , Ug′ ) with B ′ , the fixed g, and Θ (plus S0 in function mode) as Kg′ , and renders and packages the
validated (D′ , Z ′ ) with Kg′ as H′ . The pair (R′ , H′ ) becomes the starting state for the next request.
...
modification requests (30 per harness, balanced across request type and localization difficulty), and the two
evaluation arms with the SKILL.md manifest that exposes the handbook to the planner. The evaluation
...
We evaluate on two production-style agent harnesses that differ sharply in scale and language.
 ...
 • Terminus-2 is a Python terminal agent from the Harbor framework. It drives a real terminal through
 ...
 • Codex is the Rust monorepo behind the Codex coding agent. It is large and multi-surface, spanning a
   CLI, a TUI, an app-server, configuration, and sandboxing across many crates, with thousands of files and
   deep call graphs.

Following the selection rule in Section 3.2, Terminus-2 uses the function-as-leaf mode, since a reliable seed
skeleton is available and function-level entries stay within budget, whereas Codex uses the file-as-leaf mode to
...
Table 2 Phase I static facts and resulting handbook structure for the two evaluated harnesses. The upper block
reports the program graph G produced by static analysis (Appendix A); the lower block reports the built handbook.
...
entry corresponds to one file.
                  ...
                  Program graph         Internal-function nodes                 103        34,363
                  (Phase I)             Boundary nodes                           41         4,016
...
Table 3 Representative modification requests, chosen separately for each harness, with the localization challenge each
type poses.

       Terminus-2                                Codex                                     Localization challenge
Q      Require marking task_complete three       Raise exec_command’s wait ceiling to      An existing behavior must be found
       times, with an “are you sure?”            300 000 ms, keeping schema bound and      and altered; the difficulty is telling the
       re-prompt, before grading.                docs in agreement.                        target apart from similar triggers or
                                                                                           rules.
CF     Add a background flag that sends          Add a per-command env override,           The capability spans coordinated sites
       keystrokes without capturing output,      threaded through the spawn path           (schema, pipeline, interface, docs) that
       across the terminal layer and both        across both shell tools.                  must all be located together.
       formats.
SH     Mask secrets before any model             Mask secrets in every capture path and    Relevant sites hide in mirrored,
       snapshot and before writing the session   before writing the on-disk rollout.       fallback, or cold paths, so keyword
       recording.                                                                          search alone misses some of them.
...
harnesses.
Table 4 Representative modification requests at each localization-difficulty level, chosen separately for each harness,
with the localization challenge each level poses.

            Terminus-2                               Codex                                   Localization challenge
Easy        Strip ANSI escape codes from all         Add an informational comment field      One well-defined site: the change
            captured terminal output.                on both shell tools.                    maps to a single behavior that a
                                                                                             keyword or one file read can pinpoint.
Medium      Add a per-command cwd, threaded          Add an expect output check on both      Coordinated edits: one field or check
            through execution, the terminal layer,   shell tools, annotating the             must be threaded through parsing,
            and both formats.                        observation on failure.                 execution, and documentation, so
                                                                                             several related sites must be found
                                                                                             together.
Hard        Keep run counters and step history       Reconcile overlapping fields across    Indirect coupling: the sites sit on
            consistent across a mid-run              exec_command, shell_command, and       cold or mirrored paths
            compaction.                              write_stdin.                           (post-compaction bookkeeping;
...
Figure 6 The SKILL.md manifest that exposes a handbook to the planner in the Handbook-Assisted arm. The same
structure is used in both leaf modes; a leaf is a function in function-as-leaf mode and a file in file-as-leaf mode.
...
The handbook is exposed to the planner as a skill: a SKILL.md manifest tells the planner when and how to
consult the handbook, and points to the reference files it navigates. Figure 6 shows this manifest. The same
structure is used in both leaf modes; each stage’s leaf cards hold functions in function-as-leaf mode and files in
file-as-leaf mode.
...
Used in function-as-leaf Phase II to assign one function to one or more stages, as a whole or as contiguous
...
Used in file-as-leaf Phase II to summarize each scanned file. In deep mode—where the file is the handbook’s L3
...
The two modes synthesize the document tree differently. In function-as-leaf mode, each L1, L2, and L3 node is
...
file-as-leaf mode, the validated file cards from Appendix D.1 already are the L3 content, and a single-pass
...
correct_approach in place of correct_localization.
...
Each plan is scored independently against the answer key on a 0.0–5.0 scale in 0.5 steps. The three dimensions
are combined with weights 0.50 (localization), 0.25 (scope_bloat), and 0.25 (reasoning); scope_bloat
...
This appendix walks through one real Handbook-Assisted run on Terminus-2 (case Q1, DeepSeek-V4-Pro
planner), reconstructed from its recorded trace. The run makes concrete the two moves at the heart of BGPD:
the planner first routes through the handbook to decide which code sites are in scope, and then verifies each
...
is taken from the run; only whitespace is trimmed.
    ...
    Right now the model only has to mark task_complete once to reach the “are you sure?” step, then once more to
    get graded. I want a higher bar: the model must mark completion three consecutive times before we actually
...
first task is therefore one of localization — finding every site that implements the completion handshake.
    ...
    Guided by the SKILL.md policy, the planner reads the handbook top-down, narrowing the scope before opening
    any source:
    ...
    2. stages/stage-4.8.md (Completion Gate) and stages/stage-4.7.md (Command Execute): the handshake
       lives in the gate.
    3. registers.md: the gate is backed by the state register reg-pending-completion, whose read and write sites
    ...
    Because the handbook is a pure location index and stores no source, the planner now opens the real terminus_-
    2.py to confirm each locator. One search_file_content for _pending_completion returns seven occurrences,
...
Edit plan. With the sites confirmed, the plan replaces the Boolean _pending_completion with an integer
counter _completion_confirmations: the loop increments it on each completion mark, resets it on a non-
completion turn, and returns for grading only on the third consecutive mark. The change set is four modify
actions, all in terminus_2.py — initialization, per-run reset, and the loop’s two branches. The completion
