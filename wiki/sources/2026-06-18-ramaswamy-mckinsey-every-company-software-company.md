...
Auto-generated (ASR) transcript fetched via the [youtube-transcript-skill](../../.claude/skills/youtube-transcript-skill/SKILL.md) (Playwright/DOM route); light cleanup applied to proper nouns (the ASR renders "Sridhar Ramaswamy" variously as *"Sria Rama Swami / Sidar Ramaswami / Thriar"*, "Cortex Code/Coco" cleanly, "Neeva" as *"NEA"*, "McKinsey" as *"McKenzie"*). The transcript appears **doubled** (the intro repeats from ~0:00 a second time at the tail); the duplicate was disregarded at ingest. Transcript provenance (ASR) does not feed confidence per [§Lifecycle].
