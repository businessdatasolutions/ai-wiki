...
- **Does localization gain survive to executed edits?** Plan quality and localization F1 are LLM-judged against reference plans; the paper does not report whether Handbook-Assisted edit plans, once *executed*, pass the harness's own tests at a higher rate than Baseline. The obvious next experiment is end-to-end (localize → plan → execute → test) with a pass/fail outcome.
