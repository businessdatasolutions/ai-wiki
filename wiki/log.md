...
## [2026-08-12] ingest | Five videos on the enterprise-AI return gap — BBC *AI Decoded*, McKinsey GBS, Frey/Bloomberg, Miller/WorkLab, Collison/YC

Batch ingest of five YouTube sources (31 Jul – 10 Aug 2026), processed together because four of them ask the same question — *why hasn't the AI spend shown up in the numbers?* — at four different altitudes.

**Acquire.** All five fetched with the [`youtube-transcript-skill`](.claude/skills/youtube-transcript-skill/SKILL.md) Playwright path. **The run exposed three real bugs in the skill, all now fixed:**

1. **Double-trigger toggling the panel shut.** `_trigger_transcript_panel` fired *both* the `yt-action` engagement-panel event *and* a click on "Show transcript"; because YouTube's button toggles, the event opened the panel and the click immediately closed it, so `_wait_for_transcript` polled a panel that had been open ~200 ms. Two of five videos failed deterministically at 30 s, 45 s, 60 s and 90 s timeouts. Symptom is identical to the [2026-05-13 anti-bot incident](.claude/skills/youtube-transcript-skill/SKILL.md) but the cause is not — the diagnostic that separates them is `/youtubei/v1/get_transcript` returning **200** rather than 400. Fixed by making the click primary and the event a fallback. (The BBC video additionally needed one retry — mild flakiness on long videos persists.)
2. **Chapters silently collapsing.** YouTube dropped `timeRangeStartMillis` from `macroMarkersListItemRenderer`, so every chapter scraped `start_ms: 0` and `to_markdown`'s boundary filter put **all** segments under the *last* chapter heading — a well-formed-looking file with nine empty sections. The scrape now falls back to parsing the human-readable `timeDescription` label, with a defence-in-depth re-derivation in `to_markdown`. Signature to check for: a column of zeros in the YAML `start_ms`.
3. **Every segment scraped twice.** `_extract_segments` queries `ytd-transcript-segment-renderer` page-wide; when YouTube mounts a second transcript panel the whole list is collected once per panel. The Collison capture was an exact two-fold repeat (588 → 294); the Miller capture duplicated with *different line-wrapping* (712 → 356), which is why byte-equality missed it. Added `_dedupe_segments()` keyed on `(timestamp, whitespace-normalised text)`. **This retroactively explains the duplication artifact flagged-but-uncorrected on [[2026-07-19-why-netflix-is-betting-on-systems-thinkers-not-specialists-in-the-ai-era|the 19 July Netflix ingest]]**; all three failure modes are now documented in the skill's *Failure modes* section. ASR proper-noun cleanups applied per file and recorded in each raw file's `notes:`.

**Process.** Five new source pages:

- **[[2026-07-31-collison-yc-startup-school-is-ai-breaking-the-lean-startup-playbook|Collison / YC Startup School]]** — *decorrelate rather than iterate into a niche*; Stripe's business-formation data (~2× YoY, improving median, declining time-to-revenue); the cognitive L1 cache; *"what if you succeed?"*; enterprises now buying from unvalidated startups because "the risk of the status quo is actually extremely high."
- **[[2026-08-01-bbc-ai-decoded-why-isnt-ai-working-for-your-company|BBC AI Decoded]]** — the chain-and-bottleneck framing with Hare's radiology worked example; PwC's ~4,500-CEO survey (56% no ROI, one in eight claiming a saving or revenue gain); "return on intelligence" and a ~30% pilot-to-production ratio; the adoption and imagination gaps; a 3:1 tool-access-to-training ratio; hybrid/on-prem with open weights.
- **[[2026-08-03-mckinsey-agentic-ai-and-the-future-of-global-business-services|McKinsey Talks Operations / GBS]]** — 20–30% smaller but more productive; pyramid → diamond talent model and the "agent force"; automate-first vs right-shore-first; the three E's; *"you're still just automating the wrong work."*
- **[[2026-08-05-frey-bloomberg-trumponomics-why-ai-isnt-boosting-productivity|Frey / Bloomberg Trumponomics]]** — the **verification tax** ("the time saving minus the time for verification"); the computer revolution as historical control; *adoption for what?*; drill-deeper-vs-drill-more-holes; declining business dynamism; the separate-condenser framing; open weights as the sovereignty route.
- **[[2026-08-10-miller-worklab-the-ai-shift-most-companies-didnt-see-coming|Miller / Microsoft WorkLab]]** — *"AI is not a tool"* as a budgeting-and-measurement aetiology of the trap; the third (multi-agent) shift and its cost shock; the frontier unit; Gremlin mode; three hiring criteria; a 34-agent org chart; the context layer; the 1:5 people-to-tools ratio.

**Neighbour scan** (≥3 candidates, surfaced per Ingest step 5): 15 typed edges added across the five pages. The one worth flagging is a **`contradicts`** — [[2026-08-05-frey-bloomberg-trumponomics-why-ai-isnt-boosting-productivity|Frey's]] declining business dynamism and "we're seeing less entry" against [[2026-07-31-collison-yc-startup-school-is-ai-breaking-the-lean-startup-playbook|Collison's]] ~2× jump in Stripe business formation, five days apart, neither engaging the other. The `via:` records what the disagreement turns on (one payments platform's account creations vs economy-wide entry series; year-over-year vs multi-decade; formation vs survival into a durable employing firm) without adjudicating it. Considered and skipped: the AI-Index adoption data (background, not a source-to-source edge) and [[2026-07-20-bbc-researchers-studied-ai-layoffs-heres-their-warning|the July BBC episode]] as a neighbour for anything but the BBC page.

**Files touched (23).** Five new source pages; one new entity ([[Bloomberg Podcasts]], promoted on second-source rule); five entity updates ([[BBC]] +`BBC News`/`AI Decoded` aliases and a second channel, [[Microsoft]] 3→4, [[McKinsey & Company]] 13→14, [[Y Combinator]] 14→15, [[PwC]] 1→2 as a *cited* rather than authoring source); ten concept updates — [[concepts/micro-productivity-trap|micro-productivity-trap]] (40→44, four new sections), [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (90→95), [[concepts/ai-employment-effects|ai-employment-effects]] (67→70), [[concepts/durable-skills|durable-skills]] (37→39), [[concepts/systems-thinking|systems-thinking]] (6→7, confidence 0.87→0.90), [[concepts/automation-vs-augmentation|automation-vs-augmentation]] (54→55), [[concepts/ai-agents|ai-agents]] (21→22), [[concepts/ai-deskilling|ai-deskilling]] (14→16), [[concepts/open-source-ai|open-source-ai]] (2→4, confidence 0.72→0.80 — the first two *non-vendor* sources on that page), [[concepts/dynamic-capabilities|dynamic-capabilities]] (24→26); plus `index.md`, this log, and the skill fixes.

**Epistemic note.** This batch is unusually vendor-heavy: two of the three BBC guests sell the remedy under discussion, the McKinsey episode is first-party content about the firm's own service line, and the Miller episode is vendor-published from the vendor's own customer conference. The *diagnosis* is corroborated across all of them and against the wiki's existing non-vendor sources; the *numbers* mostly are not, and the concept pages say so. A reported OpenAI/Hugging Face security incident asserted on the BBC programme is recorded on the source page as an on-air claim only and deliberately **not** propagated to any concept page. qmd re-embedded.

## [2026-07-22] refactor | Open thread — *harness-evolution-validation-frontier*: is measured harness lift validated against executed, tested outcomes?

Opened [[threads/harness-evolution-validation-frontier|harness-evolution-validation-frontier]] (status: open) to consolidate a validation gap that recurs across the harness-evolution cluster but was scattered where no reader saw it whole: harness-evolution methods report a lift at whatever proxy is cheapest to score (plan quality, milestone completion, LLM-judged localization), and the executed-and-tested end-to-end number is mostly missing or second-hand. Triggered by the [[2026-07-14-wang-shi-harness-handbook-behavior-localization|Harness Handbook]] executed-edits flag; gathers the sibling open questions from [[2026-05-11-karten-zhang-continual-harness-online-adaptation|Karten]] (Pokémon-only capability floor), [[2026-03-26-pan-natural-language-agent-harnesses|Pan]] and Meta-Harness (both carried second-hand; primary-source ingest open), and [[2026-03-05-lou-deepmind-autoharness-code-harness-synthesis|Lou/AutoHarness]]. Resolution plan: a validation-tier synthesis once an end-to-end localize→plan→execute→test benchmark or the two open primary-source ingests (arXiv:2603.25723, arXiv:2603.28052) land. Files touched: new thread page; `index.md` (Threads section); Harness Handbook source (open-question now points at the thread); this log. No content-page claims changed — this is a tracking artifact, not a synthesis.

## [2026-07-22] ingest | Wang, Shi et al. — *Harness Handbook*: behavior localization as the named prerequisite of harness evolution (acquired via Zotero)

Ingested **arXiv:2607.13285v1** (cs.AI, 14 Jul 2026), *Harness Handbook: Making Evolving Agent Harnesses Readable, Navigable, and Editable* by **Ruhan Wang** & **Yucheng Shi** et al. (primarily **Tencent HY LLM Frontier** + Indiana University; 10 authors). **Acquire**: pulled from the local Zotero `ai-wiki` collection via `zotero-acquire` (item `JITW7D7E`, `preprint` → `raw/papers/`, `pdftotext` conversion, 29 pp); routing confirmed correct at Process. **Pre-flight**: full paper (29-page PDF, 1811-line extract — not a sample); identity verified (arXiv 2607.13285, Tencent/IU). The wiki's first source to isolate **behavior localization** (finding *all* code sites implementing a described behavior) as the central bottleneck of **harness evolution** — the prerequisite every edit-generation approach ([[2026-03-30-lee-meta-harness-end-to-end-optimization|Meta-Harness]], [[2026-05-11-karten-zhang-continual-harness-online-adaptation|Continual Harness]]) assumes solved. Contributes the **Harness Handbook** representation (static analysis + LLM structuring; L1→L3 tree + state-register view; progressive disclosure; auto-resync on every non-empty diff) and **BGPD** workflow; evaluated on **Codex** + **Terminus-2** coding harnesses (win rate 38.3/28.3 and 45.6/26.7 at no extra tokens; weaker planner matches Opus 4.8 / GPT-5.5 reference localization). **Neighbour scan** (harness-evolution cluster, ≥3 candidates surfaced): `supports` edges to Karten (added reciprocal edge + open-question closure note on the Karten source), Pan (NL harnesses), Lee/Khattab (Meta-Harness), Lou (AutoHarness); considered-but-skipped: Sinclair/SEI-CMU (AI-native SE — too broad). **Files touched**: new source page; [[concepts/agent-harness|agent-harness]] concept (source_count 74→75, `last_confirmed` 2026-07-22, new §Behavior localization); Karten source (reciprocal edge + open-question update); `index.md`; this log. **No W&W tags** (methodological/LLM-internals, per academic-harness-paper precedent). All 10 authors dangling (single-source). qmd re-embedded.
...
**[[2026-07-19-why-netflix-is-betting-on-systems-thinkers-not-specialists-in-the-ai-era]]** — Elizabeth Stone (Chief Product and Technology Officer, [[Netflix]]), her second [[Lenny's Podcast]] appearance, 2.5 years after her first (as CTO). Headline claim: *"we need more systems thinkers in a world with AI... people who can look across all the business domains."* Netflix is hiring fewer narrow specialists and more generalists — with an explicit carve-out for scarce-domain specialists (playback/encoding) — and growing central/core-infrastructure engineering to build shared **paved paths**, motivated directly by agents needing source-of-truth data and consistent scaffolding across systems. A universal, non-level-specific **AI-fluency career-ladder overlay** applies from new hires to the most senior leadership, including hiring-process changes (AI-tool use now permitted in coding interviews). Culture doctrine **"excellence as an operating system"** (talent density, high agency/autonomy, comfort with risk-taking, resistance to adding process, blameless retros) is explicitly likened to how "the top AI labs operate." The keeper's test continues as a mostly-positive feedback ritual. Netflix continues intern/new-grad hiring with increased craft-mastery mentorship investment. AI use beyond coding: distillation of decades of institutional experiments/insights; content-production tooling via the Interpositive (Ben Affleck) acquisition. Netflix Prize cited as a nearly-two-decade AI/ML credibility anchor. **Data-quality note**: the raw transcript file contains the full interview text twice in exact sequence — a fetch-time duplication artifact, not two segments; read once, flagged for future ingests.

**Dynamic-capabilities tags** (4): `digital-transforming/redesigning-internal-structures` (hiring-mix shift + AI-fluency career-ladder overlay + hiring-process changes), `digital-transforming/improving-digital-maturity` (paved-paths infrastructure + institutional-knowledge distillation), `strategic-renewal/organizational-culture` ("excellence as an operating system"), `digital-seizing/rapid-prototyping` (PMs/designers/data-scientists prototyping further into the product lifecycle before an engineering handoff).

**Neighbour-source scan** (5 candidates surfaced, all accepted, all `supports`, one-directional from the new source per CLAUDE.md's "encouraged not required" reciprocal-edge guidance — scope-managed given the number of concept/entity pages this ingest already touches): [[2025-07-02-joshi-venkatraman-fowler-expert-generalists|Fowler's Expert Generalist]] — Stone's generalist-with-a-narrow-specialist-carve-out hiring pattern independently reaches Fowler's thesis without citing the term. [[2026-04-21-forsgren-macvean-build-core-skills-thrive-ai-era-developer|Forsgren & Macvean/Google I/O]] — both independently name systems thinking/system design as the rising AI-era engineering-altitude skill. [[2026-07-10-building-the-future-of-agentic-infrastructure|Claude Platform team]] — Stone's paved-paths-become-more-important claim converges with the Claude Platform team's agent-identity/scoped-access-hardening account from the harness-thinning-what-persists synthesis. [[2026-07-09-nyc-executive-forum-2026-a-leaders-guide-to-agentic-ai|Vachhrajani/AWS]] — same infrastructure-hardening convergence, enterprise-operator vs. AWS-advisory vantage. [[2026-07-14-khan-academy-ceo-the-real-ai-opportunity-is-in-boring-industries-sal-khan|Khan Academy]] — both describe designers/PMs/data-scientists gaining fluid prototyping-and-shipping capability that used to require an engineering handoff.

**Cross-cutting updates**: [[concepts/systems-thinking|systems-thinking]] (5→6 sources, confidence 0.82→0.87; new hiring-criteria-altitude section — the widest altitude spread yet for the concept), [[concepts/durable-skills|durable-skills]] (36→37; new section on AI fluency as a universal career-ladder expectation — a seventh independent naming of the durable-skills vocabulary cluster), [[concepts/expert-generalist|expert-generalist]] (3→4 sources, confidence 0.75→0.8 — the first source to corroborate the underlying claim *without* citing Fowler's term, breaking the prior vendor-propagation cap), [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (88→89; new section on paved-paths infrastructure hardening as an enterprise-operator instance of the [[syntheses/harness-thinning-what-persists|harness-thinning-what-persists]] synthesis, plus culture-as-adoption-enabler), [[concepts/dynamic-capabilities|dynamic-capabilities]] (22→23; new subsection — the wiki's first streaming-media-incumbent W&W instance), [[concepts/ai-employment-effects|ai-employment-effects]] (64→65; new section — a seventh reshape-over-replace voice, plus continued junior-talent investment as a second practitioner counter-example to the entry-level-decline pattern). [[Lenny's Podcast]] (6→7 sources; the channel's first large-incumbent-CPTO register). **Created [[Netflix]]** entity (first appearance, central subject — precedent: [[Khan Academy]], [[NVIDIA]], [[PwC]]). Dangling (single-source, deferred, per the person-entity second-source rule): Elizabeth Stone.

**Debates flagged, no supersession**: (1) Stone's human-talent generalist-hiring shift runs on a different axis than the wiki's existing model-specialize-after-frontier decision rule (Nadella/Huang) — flagged on the source page and in [[agent-harness]]'s adjacent territory to prevent future conflation of the two axes. (2) Netflix's continued intern/new-grad hiring, alongside [[2026-07-09-nyc-executive-forum-2026-a-leaders-guide-to-agentic-ai|Vachhrajani/AWS's]] Matt Garman anecdote, is a second practitioner counter-example (same month) to [[2026-04-28-brynjolfsson-canaries-coal-mine|Brynjolfsson et al.'s]] entry-level-decline ADP data — both anecdotal against a large-N study, flagged as an open question in [[concepts/ai-employment-effects|ai-employment-effects]]'s Debates section rather than treated as a refutation.
...
**[[2026-07-10-building-the-future-of-agentic-infrastructure]]** — a panel of **Jess Yann** (PM, Claude Managed Agents, moderating), **Katelyn Lesse** (Head of Engineering, Claude Platform), and **Angela Jiang** (Head of Product, Claude Platform), on Anthropic's own **Claude** YouTube channel, 10 Jul 2026. The wiki's **first first-party Claude Platform interview** (as distinct from Anthropic's engineering-blog register). Six threads: **agent identity** as an outcome-negotiation model (the agent requests scoped access and effectively provisions its own auditable service account); **agent-to-agent communication** as plain API or a thin MCP server, called "the same way that a person might interact with that agent"; **thinner harnesses and "meta harnesses"** — Jiang's account of harnesses shrinking as models improve, and teams combining strategies (competitive multi-agent search, adversarial generator/critic pairs, an advisor "call a friend" strategy) before converging on one framework; worked examples (a hackathon industrial-knowledge-capture system, Shopify's "River" end-to-end agentic dev platform); **barriers** (security/compliance, evals) and a **staged ROI model** (individual speed → team → cross-team process, with financial/user metrics deferred); two named **failure modes** (hyper independence, sprawl); and a closing vision of agents as an **invisible substrate**, "more like an operating system," anchored by the shipped **outcomes** primitive (rubric + spend budget).

**Dynamic-capabilities tags** (2): `digital-transforming/redesigning-internal-structures` (the engineering-team reshaping account — same headcount, end-to-end design opinions, orchestrating "Claudes"), `digital-seizing/rapid-prototyping` (the staged individual-first ROI-measurement discipline + the hackathon worked example). Judged content-first per the schema's caution against forcing the W&W lens onto pure technology/product content — the ROI-staging and engineering-team-reshaping claims are load-bearing organizational claims, not merely product description, matching the precedent set on video 6.

**Neighbour-source scan** (4 candidates surfaced, all accepted, all `supports`, reciprocal): [[2026-07-08-jensen-huang-why-companies-need-open-agent-systems|Huang/NVIDIA (video 6)]] — published two days earlier, independently describes the same harness-thinning-as-models-improve mechanism and names a thin MCP server as the agent-to-agent interface, from substrate-vendor vs. first-party-platform vantages. [[2026-05-07-anthropic-managed-agents-decoupling-brain-hands|Anthropic Managed Agents engineering blog]] — this video is the Claude Platform team's own direct continuation of that April 2026 post (same service, now discussed with agent identity, the outcomes primitive, and "dreaming"). [[2026-05-08-bratanic-unified-agentic-memory-hooks|Bratanic's dream-phase pattern]] — Jiang's passing mention of "dreaming" as a Claude Managed Agents concept independently names the same offline-distillation architecture Bratanic documented from outside Anthropic. [[2026-06-17-priest-atlantic-pwc-ai-agents-changing-business|Priest/PwC]] — both frame agents becoming invisible operating-system-like infrastructure rather than discrete tools, from platform-vendor and enterprise-operator vantages respectively. Considered, not linked: [[2026-05-09-chase-agent-development-lifecycle|Chase/ADLC]] and [[2026-05-21-chase-langchain-interrupt-26-future-of-ai-agents|Chase/Interrupt 26]] (thematic MCP/harness overlap, no specific shared claim); [[2026-06-24-from-demo-to-production-why-agentic-ai-systems-fail|"From Demo to Production" (InfoQ)]] (shares MCP/orchestration vocabulary but addresses a different failure-mode layer).

**Cross-cutting updates**: [[concepts/agent-harness|agent-harness]] (73→74; new section — thinning-harness mechanism narrated from inside the vendor, the meta-harness taxonomy, agent identity as a Constraints-layer primitive, MCP-as-agent-interface convergence with Huang), [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (87→88; new section — the staged ROI model and the two named failure modes), [[concepts/ai-employment-effects|ai-employment-effects]] (63→64; new section — the engineering-team reshaping account as a sixth reshape-over-replace voice), [[concepts/dynamic-capabilities|dynamic-capabilities]] (21→22; new subsection). [[Anthropic]] (23→24 sources; Claude Managed Agents entry extended with the outcomes/dreaming/agent-identity update and this source's three named speakers). Reciprocal `supports` edges + body-wikilink twins added to all four neighbour source pages ([[2026-07-08-jensen-huang-why-companies-need-open-agent-systems|Huang/NVIDIA]], [[2026-05-07-anthropic-managed-agents-decoupling-brain-hands|Managed Agents blog]], [[2026-05-08-bratanic-unified-agentic-memory-hooks|Bratanic]], [[2026-06-17-priest-atlantic-pwc-ai-agents-changing-business|Priest/PwC]]). Dangling (single-source, deferred, per the person-entity second-source rule): Jess Yann, Katelyn Lesse, Angela Jiang.

This closes the same-day-origin 7-video batch (Khan Academy → AWS re:Invent 2025 → MIT Sloan "Event to Ecosystem" → MIT Sloan Ep.16 → AWS NYC Executive Forum → Jensen Huang/NVIDIA → this video).

## [2026-07-15] ingest | Jensen Huang (NVIDIA) and Harrison Chase (LangChain) on companies built on harnesses, and the Deep Agents + OpenShell blueprint

A user-supplied YouTube URL (video 6 of a same-day-origin 7-video batch), acquired via `youtube-transcript-skill` and processed in this session.

**[[2026-07-08-jensen-huang-why-companies-need-open-agent-systems]]** — NVIDIA CEO **Jensen Huang**, interviewed by **[[Harrison Chase]]** (LangChain co-founder/CEO) on the [[LangChain]] YouTube channel, 8 Jul 2026. The wiki's first **substrate/silicon-vendor** vantage on [[concepts/agent-harness|agent-harness]] and [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]]. Headline claim: *"today, most companies are built on business processes. In the future, most companies will be built on harnesses."* Doubles as the launch of a joint **NVIDIA + LangChain "Deep Agents + OpenShell" blueprint** — Deep Agents running Nemotron 3 Ultra inside OpenShell, a secure open runtime, deployable cloud/on-prem/DGX. Contributions: a three-lever specialization account (model + harness-tuning + prospective post-training-inside-the-harness) anchored by a named benchmark (Nemotron 3 Ultra 86% vs. Claude Opus 87%, ~10x cheaper); a specialize-after-frontier decision rule ("start with the frontier... as soon as it gets good enough, specialize") independently convergent with [[2026-06-05-nadella-hoffman-possible-ai-future-of-the-firm|Nadella's]] "don't use frontier models for non-frontier problems"; runtime security/access control framed as the actual deployment gate, via an employee-onboarding analogy ("an HR system, if you will, for AI") that sits in tension with Huang's own explicit anti-anthropomorphizing stance ("it's electrons, not atoms... a tool, like my vacuum cleaner"); and a fifth independent reshape-over-replace employment claim ("the more AI we use, somehow the more people we have to hire" — new job categories: evals, benchmarks, guardrails).

**Dynamic-capabilities tags** (2): `digital-transforming/redesigning-internal-structures` (companies built on harnesses, not business processes), `digital-seizing/rapid-prototyping` (the Deep Agents + OpenShell blueprint as a reusable rapid-build starting point). Judged content-first per the schema's caution against forcing the W&W lens onto pure technology/infrastructure sources — the "companies built on harnesses" thesis and the blueprint's rapid-deployment framing are load-bearing organizational claims, not merely product description, so tagging was applied rather than skipped.

**Neighbour-source scan** (5 candidates surfaced, all accepted, all `supports`, reciprocal): [[2026-05-09-chase-agent-development-lifecycle|Chase/ADLC]] and [[2026-05-21-chase-langchain-interrupt-26-future-of-ai-agents|Chase/Interrupt 26]] — same interviewer/company; Huang's post-training-inside-the-harness account and Nemotron benchmark extend Chase's frameworks/runtimes/harnesses and model/harness/context layering. [[2026-06-05-nadella-hoffman-possible-ai-future-of-the-firm|Nadella/Hoffman]] — independent vendor-CEO (Microsoft vs. NVIDIA) convergence on the identical frontier-vs-specialized-model decision rule, one month apart. [[2026-05-06-kropp-bcg-hbr-dont-treat-ai-agents-like-employees|Kropp et al./BCG]] — shared anthropomorphizing-AI topic; Huang's stated position aligns with Kropp's anti-employee-framing thesis even as his own HR-system/onboarding language for access control sits in tension with it. [[2026-07-09-nyc-executive-forum-2026-a-leaders-guide-to-agentic-ai|Vachhrajani/AWS NYC]] — two different vendor stacks (OpenShell vs. Bedrock AgentCore) converging on runtime security/access control as the enterprise-deployment precondition. Considered, not linked: [[2026-06-17-ng-langchain-interrupt-future-of-ai-agents|Ng/LangChain Interrupt]] (same LangChain-channel cluster, shares an `open-weight-models` tag, but no specific shared claim beyond thematic proximity).

**Cross-cutting updates**: [[concepts/agent-harness|agent-harness]] (72→73; new section — companies-built-on-harnesses thesis, specialization levers, specialize-after-frontier decision rule, runtime security/access control), [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (86→87; new section + a Debates bullet on the anti-anthropomorphizing-rhetoric-vs-employee-onboarding-practice tension), [[concepts/ai-employment-effects|ai-employment-effects]] (62→63; new section on the fifth reshape-over-replace voice + Chase's automate-the-past-vs-enable-the-impossible framing), [[concepts/dynamic-capabilities|dynamic-capabilities]] (20→21; new subsection). [[LangChain]] (7→8 sources; new "Deep Agents + OpenShell blueprint" product entry + "Partners" section), [[Harrison Chase]] (3→4 sources; first appearance as interviewer rather than headline speaker). **Created** [[NVIDIA]] entity (first appearance, central subject — precedent: [[Khan Academy]], [[PwC]]; NVIDIA had previously surfaced only in passing, as silicon named alongside AMD, in [[2026-06-05-nadella-hoffman-possible-ai-future-of-the-firm|Nadella/Hoffman]]). Dangling (single-source, deferred, per the person-entity second-source rule — central-subject status does not itself trigger promotion for individuals, precedent: Sal Khan on the Khan Academy source): Jensen Huang.

## [2026-07-15] ingest | NYC Executive Forum 2026's "A Leader's Guide to Agentic AI" — the mental-model-shifts framework's originator, in his own words

A user-supplied YouTube URL (video 5 of a same-day-batch-origin 7-video set), acquired via `youtube-transcript-skill` and processed in this session.

**[[2026-07-09-nyc-executive-forum-2026-a-leaders-guide-to-agentic-ai]]** — Ishit Vachhrajani (AWS Director of Technology and AI, Executive in Residence), sole speaker at the AWS NYC Executive Forum, 9 Jul 2026. **Identity check** (title strongly resembles the AWS "advanced team structures" franchise already in the wiki): confirmed a genuinely distinct talk — different title, speaker, event, and governing structure (organization/trust/culture, not USE/COMPOSE/BUILD) — but a direct authorship link: Vachhrajani is the four-mental-model-shifts framework's originator, first credited to him *secondhand* on the [[2025-12-02-allen-brozovich-davis-aws-reinvent-advanced-team-structures|re:Invent 2025]] source page (Brozovich: "a same-conference session by Ishit Vachhrajani"), and here delivers it *first-hand and in full*: silos → immune system, gates → guardrails, factory-floor → trading-floor, operational-execution → research-lab. Contributions: an accounts-payable immune-system worked example; an internal-Amazon agent-org-design anecdote (15-parallel-agents chaos → role-bounded multi-tier redesign on day 7) structurally parallel to Danske Bank's 35%-coding-time finding; an independently-reached hourglass organization; Singapore's Davos-2026 governance model + Bedrock AgentCore as trust infrastructure; a barbell research-lab culture; **Jevons' 1865 coal paradox** cited directly by name and date (new primary-source grounding for a mechanism the wiki already tracked unattributed); **Andrej Karpathy's "job is a bundle of task plus glue"** framing (new to the wiki); a third independent citation of the Anthropic Build-with-Claude hackathon's non-developer-wins evidence; an AWS marketing "AIR" agentic-recommendation-engine case (90-day concept-to-GA); a live demo of "Quick" (an internal AWS AI chief-of-staff agent); and a token-maxing → velocity-maxing → yield-maxing measurement framework for the adoption journey.

**Dynamic-capabilities tags** (6): `digital-sensing/digital-scenario-planning`, `digital-seizing/rapid-prototyping`, `digital-transforming/redesigning-internal-structures`, `digital-transforming/improving-digital-maturity`, `strategic-renewal/organizational-culture`, `contextual/external-triggers`.

**Neighbour-source scan** (3 candidates surfaced, all accepted, all `supports`, reciprocal): [[2025-12-02-allen-brozovich-davis-aws-reinvent-advanced-team-structures|re:Invent origin edition]] — the framework's originator delivering it first-hand, strongest edge (0.75). [[2026-05-21-allen-aws-london-exec-forum-agentic-team-structures|London edition]] — shared hourglass/Singapore-governance/AgentCore/expert-generalist/non-determinism content (0.6). [[2026-06-12-aws-leaders-guide-advanced-team-structures-agentic-world|Sydney edition]] — identical Anthropic Build-with-Claude hackathon citation + shared hourglass/Singapore content (0.55). All three treated as close-cousin franchise siblings, **not** supersession — the NYC talk's title, speaker, event, and governing structure are all genuinely different from the "advanced team structures" lineage.

**Cross-cutting updates**: [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (85→86; new section — organization/trust/culture model, immune-system AP example, internal-Amazon agent-restructuring anecdote, AWS marketing AIR case, Quick demo, token/velocity/yield-maxing framework), [[concepts/dynamic-capabilities|dynamic-capabilities]] (19→20; new subsection on the independent second reach for the same organizational shape), [[concepts/ai-employment-effects|ai-employment-effects]] (61→62; new section on Jevons' 1865 citation + Karpathy's task-plus-glue framing + the third hackathon citation + Matt Garman/11,000-interns datapoint), [[concepts/software-3.0|software-3.0]] (8→9; new subsection — a third independent AWS-vendor voice on non-determinism-as-feature). [[Amazon Web Services]] (7→8 sources; NYC talk added to the AWS Events keynote-series roundup as a related-but-distinct fifth talk), [[Martin Fowler]] (3→4 sources), [[Andrej Karpathy]] (7→8 sources; new "task plus glue" citation). **Created** [[Ishit Vachhrajani]] entity (second-source promotion — dangling since the re:Invent 2025 ingest, where Brozovich credited him as the mental-model-shifts framework's source). **Updated** the three franchise source pages ([[2025-12-02-allen-brozovich-davis-aws-reinvent-advanced-team-structures|re:Invent]], [[2026-05-21-allen-aws-london-exec-forum-agentic-team-structures|London]], [[2026-06-12-aws-leaders-guide-advanced-team-structures-agentic-world|Sydney]]) with reciprocal `supports` edges and body-wikilink twins; the re:Invent page's dangling list and "open questions" section were resolved to note Vachhrajani's promotion. Dangling (single-source, deferred): Matt Garman (AWS CEO, quoted secondhand — not itself a speaker of any wiki source, so not promotion-eligible), William Stanley Jevons, Josephine Teo (already dangling on the Sydney source).

## [2026-07-15] ingest | The MIT Sloan CIO Symposium's weekly podcast — organizational capability as the AI-era differentiator, the Linux Foundation tech-talent report, and the agentic-AI maturity gap

A user-supplied YouTube URL (video 4 of a same-day-batch-origin 7-video set), acquired via `youtube-transcript-skill` and processed in this session.

**[[2026-07-10-when-ai-becomes-common-organizational-capability-becomes-the-advantage-ep-16]]** — Episode 16 of the [[MIT Sloan CIO Symposium]]'s weekly podcast, co-hosted by **Allan Tate** (Executive Chair) and **Irving Wladawsky-Berger**. Title thesis: *"access to AI is no longer the differentiator... organizations that gain a lasting advantage will be those that can effectively integrate AI into their people, processes, and decision-making."* Wladawsky-Berger walks through the [[2026-05-01-lf-state-of-tech-talent-global-2026|Linux Foundation State of Tech Talent report]] — figures match closely (net hiring +26% 2025 / +31% expected 2026 / +22% 2027; n≈400; full-stack skills gap led by AI/ML engineering at 47%) — attributing part of the post-COVID pullback to a pandemic hiring bubble, and speculating (with **Erik Brynjolfsson** cited, unresolved) that entry-level softness reflects experienced-staff retention for infrastructure work. Reads at length from a **Thomas H. Davenport** Substack article making the episode's title claim explicit: *"the ability of organizations to deploy the technology effectively and get value from it matters more than the technology."* Cites **Stanford's new AI and Organization Lab** (launched ~May 2026, in the Graduate School of Business, not computer science) as independent validation that the constraint has shifted from technology to organization design. CIO advice: build measurable AI pilots, don't overpromise to non-technical executives/boards; **agentic AI lags generative AI in organizational maturity** — who builds/owns/is-accountable-for an agent remains unresolved. Closes on a China-outperforms-the-US-on-concrete-applications contrast, attributed to US preoccupation with AGI/ASI.

**Dynamic-capabilities tags** (5): `digital-transforming/improving-digital-maturity`, `digital-transforming/redesigning-internal-structures`, `digital-seizing/rapid-prototyping`, `digital-sensing/digital-scenario-planning`, `contextual/internal-barriers`.

**Neighbour-source scan** (6 candidates surfaced, 4 accepted): [[2026-05-01-lf-state-of-tech-talent-global-2026|LF State of Tech Talent (Global)]] — accepted, `supports` (reciprocal) — this episode is a direct secondary discussion of the same primary survey data. [[2026-05-28-from-event-to-ecosystem-rethinking-how-technology-leaders-build-knowledge-in-the-ai-era|Tate's MIT Sloan CIO Symposium keynote]] — accepted, `supports` (reciprocal) — same weekly-podcast franchise and co-host. [[2026-06-11-mit-smr-agentic-ai-what-leaders-wish-they-knew-sooner|MIT SMR / CIO Symposium compilation]] — accepted, `supports` (reciprocal) — same symposium ecosystem, Thomas H. Davenport continuity. [[2026-07-09-catlin-mckinsey-podcast-real-ai-advantage|Catlin/McKinsey]] — accepted, `supports` (reciprocal) — independent corroboration of the identical "organizational redesign, not technology, is the differentiator" thesis from a different venue. Considered but not linked: [[2025-12-02-allen-brozovich-davis-aws-reinvent-advanced-team-structures|Allen/Brozovich/Davis, AWS re:Invent]] (shares two W&W cells and a compatible thesis, but no shared people/quotes/data — too thin), [[2026-06-17-priest-atlantic-pwc-ai-agents-changing-business|Priest/PwC]] (shares two W&W cells but a different layer of the adoption problem).

**Cross-cutting updates**: [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (84→85; new section pairing this episode's organizational-capability thesis with Catlin/McKinsey and the Stanford AI and Organization Lab), [[concepts/ai-employment-effects|ai-employment-effects]] (60→61; new section on net-hiring corroboration + the entry-level-hiring-slowdown debate), [[concepts/micro-productivity-trap|micro-productivity-trap]] (39→40; new section on the individual-vs-enterprise value gap + the agentic-AI governance-gap addition), [[concepts/dynamic-capabilities|dynamic-capabilities]] (18→19; new subsection on the four W&W cells this episode instantiates). **Created** [[Allan Tate]] and [[Irving Wladawsky-Berger]] entity pages (both second-source promotions — both were dangling on the 2026-05-28 keynote page) and [[Thomas H. Davenport]] entity page (second-source promotion — was dangling on the 2026-06-11 MIT SMR compilation page). **Updated** [[MIT Sloan CIO Symposium]] (2→3 sources), [[The Linux Foundation]] (3→4 sources, new "discussed by a third party" appearance mode), [[Erik Brynjolfsson]] (4→5 sources, passing mention). Dangling (single-source, deferred): Stanford AI and Organization Lab (organization).

## [2026-07-15] ingest | Allan Tate's MIT Sloan CIO Symposium keynote — from adoption to orchestration, the inquiry framework, and the CIO as designer of intelligence systems

A user-supplied YouTube URL (video 3 of a same-day-batch-origin 7-video set), acquired via `youtube-transcript-skill` and processed in this session.

**[[2026-05-28-from-event-to-ecosystem-rethinking-how-technology-leaders-build-knowledge-in-the-ai-era]]** — Allan Tate (Executive Chair, MIT Sloan CIO Symposium), pre-event keynote opening the 23rd annual symposium (28 May 2026). A rare **meta-level** source: not a firm deploying AI, but a professional leadership community restructuring its own knowledge-production model around AI. Headline reframe: **"the problem is no longer adoption of technology. The problem today is orchestration of technology."** Documents the symposium's own **inquiry framework** (structured questions, competing interpretive lenses, no forced conclusions — replacing a 2024 debates experiment), born from a hallway conversation with Irving Wladawsky-Berger. Four insights: AI as a **design question, not a capability question**; a normative success/failure test (production ↑ + human understanding ↑ = success; production ↑ + understanding ↓ = "surrendering to the machine"); the constraint shifting from **execution to prioritization** ("judgment and imagination is what becomes scarce"); and **AI expands scope of work before reducing it**. Governance reframed as a design question; the CIO redefined as "designer of intelligence systems… architect of governance." The symposium's own knowledge-delivery evolution (recordings → +transcript → +event-wide synthesis → planned search-driven clip UI) is offered as a small self-referential adoption case.

**Dynamic-capabilities tags** (5): `digital-sensing/digital-scenario-planning`, `digital-sensing/digital-mindset-crafting`, `digital-transforming/redesigning-internal-structures`, `strategic-renewal/organizational-culture`, `contextual/external-triggers`.

**Neighbour-source scan** (4 candidates surfaced, 1 accepted): [[2026-06-11-mit-smr-agentic-ai-what-leaders-wish-they-knew-sooner|MIT SMR / CIO Symposium compilation]] — accepted, typed **`supports`** (reciprocal edge added to both source pages) — same 2026 MIT Sloan CIO Symposium event; this keynote frames the inquiry model the compilation's eleven practitioner takeaways were captured within. Considered but not linked: [[2026-05-07-kiron-schrage-compound-benefits|Kiron & Schrage]] (only connective tissue is a one-line forward-reference to a Schrage session, too thin for a typed edge), [[2026-06-18-dumra-mit-smr-dbs-everyone-an-innovator|Dumra/DBS]] (shares 3 W&W cells but no thematic adjacency — single-firm banking case, not a knowledge-ecosystem source), [[2026-06-25-raboresearch-ai-it-zakelijke-dienstverlening|RaboResearch]] (incidental cell overlap only).

**Cross-cutting updates**: [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (83→84; new section on orchestration-over-adoption, the human-understanding normative test, and the CIO-as-designer-of-intelligence-systems reframe), [[concepts/micro-productivity-trap|micro-productivity-trap]] (38→39; new section on the human-understanding test as a cognitive-signature escape criterion), [[concepts/dynamic-capabilities|dynamic-capabilities]] (17→18; new subsection on structured inquiry as digital-sensing at community altitude). **Created** [[MIT Sloan CIO Symposium]] entity (kind: venue; 2nd-source promotion — the same 2026 event already underlies the MIT SMR compilation). **Updated** [[Michael Schrage]] (2→3 sources; third, passing appearance — a forward-reference to a "governance as strategy" session). Dangling (single-source, deferred): Allan Tate (Executive Chair), Irving Wladawsky-Berger (identification confident on content grounds despite heavy ASR name-garbling — "Irving Vladaskki Burgerer" — flag for confirmation against a non-ASR source before promotion).

## [2026-07-14] ingest | AWS re:Invent 2025 — the origin edition of Jonathan Allen's "advanced team structures" keynote, Stephen Brozovich's org-science frameworks, and Danske Bank's PDLC overhaul

A user-supplied YouTube URL (video 2 of a same-day 7-video batch), acquired via `youtube-transcript-skill` and processed in the same session.

**[[2025-12-02-allen-brozovich-davis-aws-reinvent-advanced-team-structures]]** — AWS re:Invent 2025 senior-leaders session (SNR307), 2 Dec 2025. This is the **origin edition** of the AWS Enterprise Strategy "advanced team structures" talk lineage the wiki already holds via [[2026-05-21-allen-aws-london-exec-forum-agentic-team-structures|Allen's London edition]] (21 May 2026, which explicitly discloses "400 hours of revision" from this deck) and [[2026-06-12-aws-leaders-guide-advanced-team-structures-agentic-world|Brovich's Sydney edition]] (12 Jun 2026). **Jonathan Allen** delivers the keynote with material that largely carries forward into the later editions unchanged (MIT NANDA 95%-zero-return, Scott Galloway's "AI won't take your job, someone using AI will," builders-to-orchestrators, Martin Fowler's expert-generalist thesis) but predates USE/COMPOSE/BUILD, the "hourglass organization" term, the moats-erosion thesis, and Project Mantle. **Stephen Brozovich** — the same AWS Enterprise Strategist who later delivers the Sydney edition under the ASR spelling "Steven Brovich" — contributes four frameworks new to the wiki: the Day-After-Tomorrow work-time framework (Peter Hinssen), four mental-model shifts (Ishit Vachhrajani) — gates→guardrails, factory-floor→trading-floor, silos→immune-system, operational-execution→research-lab — a five-phase business-lifecycle organizational-alignment model, and an asymmetric human/AI resource-allocation framework across four work types. **Richard Davis**, CTO of Danske Bank, gives the wiki's first Danske-Bank-as-primary-subject case: 850 apps migrated to AWS in 18 months, 10 Big Wins, a new PDLC triggered by a 35%-of-time-on-coding finding (a clean [[micro-productivity-trap]] instance), production agents (Cloud Buddy, ~75% triage-time cut; CheckMate, hackathon-to-production), and a change-management taxonomy for employee resistance.

**Dynamic-capabilities tags** (9): `digital-sensing/digital-scenario-planning`, `digital-seizing/rapid-prototyping`, `digital-seizing/strategic-agility`, `digital-transforming/redesigning-internal-structures`, `digital-transforming/improving-digital-maturity`, `strategic-renewal/organizational-culture`, `strategic-renewal/business-model`, `contextual/external-triggers`, `contextual/internal-enablers`.

**Neighbour-source scan** (3 candidates surfaced, all accepted, all `supports`): [[2026-05-21-allen-aws-london-exec-forum-agentic-team-structures|Allen/London]] (earlier draft of the identical keynote — Allen's own disclosed origin), [[2026-06-12-aws-leaders-guide-advanced-team-structures-agentic-world|Brovich/Sydney]] (same AWS talk franchise, third edition), [[2026-06-18-dumra-mit-smr-dbs-everyone-an-innovator|Dumra/DBS]] (bank-sector digital-transformation case-study neighbour — Danske vs. DBS, both CTO/operator-altitude with production-agent evidence). Checked but not linked: [[2026-06-17-priest-atlantic-pwc-ai-agents-changing-business|Priest/PwC]] and [[2026-07-09-catlin-mckinsey-podcast-real-ai-advantage|Catlin/McKinsey]] — their hourglass-organization and reinvention-over-productivity framings are London/Sydney-edition-and-later material that does not appear in this earlier re:Invent transcript, so no defensible edge.

**Cross-cutting updates**: [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (82→83; two new sections — Brozovich's lifecycle-phase + asymmetric-allocation frameworks, and the Danske Bank case), [[concepts/micro-productivity-trap|micro-productivity-trap]] (37→38; the 35%-coding-time finding), [[concepts/dynamic-capabilities|dynamic-capabilities]] (16→17; origin-edition W&W reading), [[concepts/automation-vs-augmentation|automation-vs-augmentation]] (53→54; third independent Galloway citation + Danske's customer-side echo). [[Amazon Web Services]] (6→7 sources; franchise entry updated to span all 4 AWS "advanced team structures"/data-strategy editions). **Created** [[Jonathan Allen]] and [[Steven Brovich]] entity pages (both second-source promotions — Allen first cited dangling on the London source, Brovich first cited dangling, as "Steven Brovich," on the Sydney source); the two ASR surname spellings ("Steven Brovich" / "Stephen Brozovich") are treated as the same person on biographical-detail grounds (Amazon since August 1999, AWS Enterprise Strategist) and recorded as aliases, flagged as unverified against a non-ASR source. Updated the London and Sydney source pages' dangling lists to reflect both promotions. Dangling (single-source, deferred): Richard Davis (CTO, Danske Bank), Danske Bank (organization), Peter Hinssen (author, *The Day After Tomorrow*), Ishit Vachhrajani (mental-model-shifts framework source).

## [2026-07-14] ingest | Sal Khan (Khan Academy CEO) on job shock, Khan Academy's internal AI adoption, and the TED/ETS Content Institute

A user-supplied YouTube URL (Silicon Valley Girl podcast, published same-day as ingest), acquired via `youtube-transcript-skill` and processed in the same session.

**[[2026-07-14-khan-academy-ceo-the-real-ai-opportunity-is-in-boring-industries-sal-khan]]** — Sal Khan (Khan Academy CEO), interviewed by Marina Mogilko. Khan is writing *Job Shock*, a tonal shift toward worry from his optimistic 2023 book *Brave New Words*, triggered by a VC portfolio company automating ~80% of a Philippines-based call-center workforce. Timeline claims: ride-share/driving sees "a real dent" in 5-10 years (Waymo); 4-5% sectoral unemployment within 3-4 years, not mass unemployment; safe jobs lean into the human element (teaching, nursing, hospitality, relationship-based sales). Khan Academy's own internal AI adoption: $1.2M/year Anthropic spend (reportedly "top of the stack" per an internal Anthropic usage report), designers/PMs given full dev environments (role blending), a hackathon-vibe-coded feature shipped in a month, 50-100% subjective velocity gain, and an explicit no-AI-layoffs policy ("if we could do three times more with the same resources, we will do three times more... that would never be the catalyst for layoffs"). The **Content Institute** — a new joint venture with TED (Khan succeeds Chris Anderson as vision steward) and ETS — is an accreditation-pending, competency-based degree capped at $10,000, built around ETS's five-skill durable-skills framework (communication, collaboration, creativity, critical thinking, + leadership), assessed via group simulations and portfolios, with six launch corporate co-design partners (McKinsey, Bain, Google, Microsoft, Replit, Accenture).

**Dynamic-capabilities tags** (3): `digital-transforming/redesigning-internal-structures` (designers/PMs given full dev environments), `digital-seizing/rapid-prototyping` (hackathon-vibe-coded feature), `contextual/external-triggers` (the BPO call-center automation shock behind *Job Shock*).

**Neighbour-source scan** (4 candidates surfaced, all accepted, all `supports`): [[2026-06-17-priest-atlantic-pwc-ai-agents-changing-business|Priest/PwC]] (reshape-not-replace logic + shared `redesigning-internal-structures` cell), [[2026-04-28-brynjolfsson-canaries-coal-mine|Brynjolfsson Canaries]] (the BPO call-center example as a concrete instance of Brynjolfsson's entry-level/automation-exposed displacement pattern), [[2026-04-03-bcg-emerson-kropp-ai-will-reshape-more-jobs-than-it-replaces|BCG reshape]] (nonprofit-CEO restatement of the reshape-more-than-replace thesis, plus shared `redesigning-internal-structures` and `contextual/external-triggers` cells), [[2026-06-29-raman-wood-worklab-job-titles-dont-matter-2026|Raman/Wood]] (ETS's five-skill framework vs. the 5 C's — independent durable-skills vocabulary convergence).

**Cross-cutting updates**: [[concepts/ai-employment-effects|ai-employment-effects]] (59→60; new section on Khan's optimist-to-worried arc + Debates note), [[concepts/durable-skills|durable-skills]] (35→36; new section on the ETS framework + Content Institute as assessment-as-product), [[concepts/automation-vs-augmentation|automation-vs-augmentation]] (52→53; new section on Khan Academy's demand-elasticity-driven augmentation-first stance), [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (81→82; new section on the nonprofit-sector adoption case), [[concepts/dynamic-capabilities|dynamic-capabilities]] (15→16; new subsection under redesigning-internal-structures), [[Anthropic]] (22→23; new customer bullet). **Created** [[Khan Academy]] entity (first appearance, central subject — precedent: [[PwC]]). Dangling (single-source, deferred): Sal Khan, Marina Mogilko, Silicon Valley Girl (channel).
...
**[[2026-06-17-priest-atlantic-pwc-ai-agents-changing-business]]** — Dan Priest (Chief AI Officer, [[PwC]]), interviewed by Nicholas Thompson (CEO, The Atlantic) on *The Most Interesting Thing in AI* (branded content, produced in collaboration with PwC). Concrete task-length ceiling for agent autonomy (~2.5 hrs general work vs. ~12–13 hrs coding before success drops below 50%); back-office-first deployment sequencing (code gen → customer service → middle office, with a Southwest Airlines case study); "adoption moves at the speed of trust, not technology"; "the best talent gets the most from AI" (a claim in tension with the wiki's established equalizing-effect literature — flagged in [[concepts/automation-vs-augmentation|automation-vs-augmentation]]'s Debates section); PwC's firm-wide GPT compounding its transformation-practice learnings; the **hourglass organization** model — a second independent voice after [[2026-05-21-allen-aws-london-exec-forum-agentic-team-structures|Allen/AWS]] converging on the same org shape.

**[[2026-07-09-catlin-mckinsey-podcast-real-ai-advantage]]** — Tanguy Catlin (Senior Partner, [[McKinsey Global Institute|MGI]] Director), interviewed by [[Roberta Fusaro]] on **The McKinsey Podcast**, published the same day it was ingested. Explicitly restates *[[2026-05-03-rewired-second-edition-sample|Rewired]]*'s core prescription in interview form; a three-lever theory of future competitive advantage (proprietary data, habit-embedding, "metabolic rate of learning"); the knowledge-based → outcome-based organization shift; a strategy/technology/people three-lens CEO diagnostic (people lens hardest); ~53% of worker activities automatable but few whole jobs are — a third independent framing (after BCG and Anthropic) of reshape-over-replace, now in [[concepts/ai-employment-effects|ai-employment-effects]].

**ASR cleanup note worth flagging for future ingests**: both transcripts carried a previously undocumented DOM-scraping artifact from the `youtube-transcript-skill` — YouTube's screen-reader-oriented spoken-form timestamps ("1 minute, 3 seconds") and, on the Priest video, inline "Chapter N: ..." chapter-marker text, both captured as if they were spoken dialogue (213 + 145 occurrences respectively). Stripped at ingest time via regex; worth adding to the skill's known-failure-modes list if it recurs on future ingests.

**Cross-cutting updates**: [[concepts/jagged-frontier|jagged-frontier]] (15→16), [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (79→81, two new framework-level sections), [[concepts/automation-vs-augmentation|automation-vs-augmentation]] (51→52, new Debates entry), [[concepts/ai-employment-effects|ai-employment-effects]] (56→57), [[concepts/dynamic-capabilities|dynamic-capabilities]] (13→15), [[syntheses/organizational-frameworks-for-ai-adoption|organizational-frameworks-for-ai-adoption]] synthesis (14→16 sources, refreshed). **Created** [[PwC]] entity (first appearance as a central subject) and [[Lucia Rahilly]] entity (promoted on her third sign-off-only McKinsey Podcast mention, following the MGI 3rd-mention threshold precedent rather than deferring indefinitely). **Updated** [[McKinsey & Company]] (12→13 sources) and [[Roberta Fusaro]] (2→3 sources, third McKinsey Podcast appearance). Dangling (single-source, deferred): Dan Priest, Nicholas Thompson, Tanguy Catlin.
...
A ~25:30 McKinsey Podcast interview: **Jason Bello** (Senior Partner) in conversation with **[[Roberta Fusaro]]** (Editorial Director) on new McKinsey research into corporate venture building. Headline claims: companies building 3+ ventures simultaneously dramatically outperform one-shot builders; average break-even cost fell $125M (2024) → $77M (2025); a two-flavor taxonomy of AI in venture building ("AI as a business" vs. "AI in the background"); milestone-tranche funding; a fact-based, blame-free culture as the enabling condition for fast pivots; disruption itself as the argument *for*, not against, venture building.

- New source page: [[2026-07-01-bello-mckinsey-podcast-serial-builder-advantage]] (`kind: video`; raw at `raw/videos/the-serial-builder-advantage-why-repeat-innovators-win.md`; ASR captions).
- `dynamic_capabilities:` tags: `digital-seizing/balancing-digital-portfolios`, `digital-seizing/rapid-prototyping`, `strategic-renewal/organizational-culture`.
- Neighbour-source scan: tag-overlap search returned 65+ candidates (the three tagged cells are broad); narrowed by content (corporate venture building / portfolio innovation / fact-based pivot culture) to three defensible edges — surfaced to the user before commit.
- Typed `supports` edges: [[2026-06-18-dumra-mit-smr-dbs-everyone-an-innovator]] (same portfolio + milestone-tranche-funding mechanism, cross-firm quantified claim vs. single-firm lived instance), [[2026-06-25-carroll-stanford-gsb-making-organizational-culture-great]] (fact-based culture practitioner claim vs. academic culture-as-social-control-system theory), [[2024-04-18-caldwell-lennys-podcast-lessons-1000-yc-startups-tarpit-ideas]] (fact-based fast pivoting, cross-context parallel — external YC startups vs. internal corporate ventures).
- Updated [[concepts/dynamic-capabilities|dynamic-capabilities]] (11→12 sources; new subsection — the wiki's first cross-firm quantified case for `balancing-digital-portfolios`, vs. prior single-case narrations).
- Updated [[McKinsey & Company]] entity (11→12 sources; new `authored-by` edge; new `employs` edge to [[Roberta Fusaro]]).
- **Promoted [[Roberta Fusaro]]** to an entity page — her second appearance as a named McKinsey Podcast host/editorial figure (first: body-prose credit on [[2026-06-18-ramaswamy-mckinsey-every-company-software-company]]; here: active on-camera interviewer).
- Dangling (deferred, single-source): Jason Bello (interviewee); Lucia Rahilly (named in sign-off outro on two episodes now, but role as active host vs. fixed co-host credit remains unclear — deferred pending clarification rather than promoted on a technicality).
- `index.md` updated: new source entry, new entity entry (alphabetical, under R).

## [2026-07-01] ingest | McKinsey *"AI Is Supercharging Software Development. Humans Determine Its Impact."* (24 Jun 2026)

Acquired via subagent dispatch (parallel fetch alongside two other videos this session) using the youtube-transcript-skill's `fetch_transcript.py --json` route; the raw fetch captured the transcript panel in two overlapping scroll passes (a known quirk), 1,102 raw segments deduplicated by (timestamp, normalized text) to 551 unique segments. Processed the same session. A ~22-minute McKinsey panel (**Janaki Palaniappan**, **Martin Harrysson**, **Matt Linderman**) on why AI coding-agent productivity gains fail to convert to organizational value without workflow/role redesign, which software-engineering skills are becoming obsolete vs. durable, and a claim that AI-generated code has so far often been more verbose and less secure than human-written code.

- New source page: [[2026-06-24-mckinsey-ai-supercharging-software-development]] (`kind: video`; raw at `raw/videos/ai-is-supercharging-software-development-humans-determine-its-impact.md`; ASR captions).
- No W&W tags applied at the concept level (the source itself touches `digital-transforming/*` per its frontmatter, but the ingest treated this as a software-engineering-organization source rather than routing further W&W tagging work).
- Typed `supports` edges: [[2026-05-02-dutt-chatterji-ai-experimentation-to-transformation]] (same workflow-redesign-over-tool-adoption diagnosis, software-engineering-specific restatement), [[2026-04-21-forsgren-macvean-build-core-skills-thrive-ai-era-developer]] (obsolete vs. durable SWE skills, McKinsey-consulting vantage vs. Google-DORA vantage).
- Concept updates: [[concepts/micro-productivity-trap]] (new software-engineering-specific restatement subsection; 36→37), [[concepts/ai-deskilling]] (new consulting-panel obsolete-skills subsection; 13→14), [[concepts/responsible-ai]] (new AI-code-verbosity/security subsection; 18→19). All three dates → 2026-07-01; none lifted past existing confidence ceilings (single qualitative consulting-panel source, unquantified beyond one anecdote).
- Entity update: [[McKinsey & Company]] (10→11 sources; new `authored-by` edge — the wiki's first source from this specific software-engineering-focused panel format, distinct from *Rewired*, the Sternfels self-narrative, the MGI reports, and the McKinsey-Podcast Ramaswamy interview already held).
- No entities promoted: Janaki Palaniappan, Martin Harrysson, Matt Linderman are single-source mentions (Dangling) per the second-source promotion rule.

## [2026-07-01] ingest | Mark Pincus (Zynga) — *"The hidden pattern behind successful products"* (Lenny's Podcast, 14 Jun 2026)

Acquired via subagent dispatch (parallel fetch alongside two other videos this session); the raw fetch captured the transcript panel in two overlapping scroll passes, 1,840 raw segments deduplicated by (timestamp, normalized text) to 920 unique segments. Processed the same session. A ~99-minute Lenny's Podcast interview with **Mark Pincus** (Zynga founder), timed to his book *Life at the Speed of Play*. Central framework: **"Proven, Better, New"** — earn the right to innovate by first nailing a proven pattern, make it decisively better, only then add something new. Sub-threads: instincts-right-95%/ideas-wrong-75%, being less ambitious as the path to bigger outcomes, "kill hope before hope kills you," AI as a "failure machine," and a "make everyone a CEO" / "stay close to the metal" management philosophy.

- New source page: [[2026-06-14-pincus-lennys-podcast-hidden-pattern-behind-successful-products]] (`kind: video`; raw at `raw/videos/the-hidden-pattern-behind-successful-products-mark-pincus.md`; ASR captions).
- **Lighter-touch ingest, per explicit user-confirmed plan**: no `dynamic_capabilities:` tags (the W&W digital-transformation lens does not genuinely fit a founder/product-craft interview), single typed relationship, single concept touch.
- Typed `supports` edge: [[2025-07-02-joshi-venkatraman-fowler-expert-generalists]] (hands-on engagement over abstraction, founder vantage vs. software-practitioner vantage).
- Concept update: [[concepts/strategy]] (new "Proven, Better, New" subsection; 7→8; date → 2026-07-01, no confidence change — single-source book-promotional interview).
- Entity update: [[Lenny's Podcast]] (5→6 sources; new `published-by` edge + sources table row).
- No entities promoted: Mark Pincus is a single-source mention (Dangling); Zynga flagged as a candidate entity, deferred.
- One deliberate non-edge: considered a typed relationship to [[concepts/expert-generalist]] but kept it to a body cross-reference only (that page's strict vendor-propagation-cap discipline requires sources to use its specific vocabulary before counting toward confidence — Pincus doesn't use "Expert Generalist" terminology), same pattern as the WorkLab/"onlyness" precedent earlier this session.

## [2026-07-01] ingest | Jesse Vincent (creator of Superpowers) — CodeRabbit *"Fixing AI Slop"* (8 Jun 2026)

Acquired via subagent dispatch (parallel fetch alongside two other videos this session); no true scroll-duplication occurred (0 repeated (ts, text) keys across 654 raw segments), but 637 of 654 segments carried a YouTube screen-reader duration-announcement artifact and 14 carried an adjacent chapter-heading DOM node's text — both mechanically stripped (whitespace-level cleanup, not a content edit). Processed the same session. A ~79-minute CodeRabbit (*The Merge*) interview with **Jesse Vincent** — creator of the open-source **Superpowers** agentic-coding framework (221k+ GitHub stars), the same skill framework loaded into this session's own Claude Code harness. Central thesis: manage AI coding agents like "enthusiastic, sleep-deprived MIT interns," not deterministic software.

- New source page: [[2026-06-08-vincent-coderabbit-fixing-ai-slop-managing-agents-like-mit-interns]] (`kind: video`; raw at `raw/videos/fixing-ai-slop-how-to-manage-agents-like-mit-interns-w-jesse-vincent-creator-of-superpowers.md`; ASR captions).
- No W&W tags (agentic-coding-tooling source, outside the digital-transformation lens).
- Typed `supports` edges: [[2026-05-07-chatterjee-anatomy-of-agent-harness]] (independent convergence on single-mandate separation between implementing and reviewing agents), [[2026-05-07-kokane-agent-harness-vs-systems-design]] (harness engineering as mature management/systems discipline applied to a new substrate).
- Concept updates: [[concepts/agent-harness]] (new "Superpowers architecture" subsection — Coordinator/Coder/Spec-Reviewer/Quality-Reviewer, adversarial reviewers with stakes, "latent space engineering"; 69→70), [[concepts/agentic-engineering]] (new convergent-evolution-of-a-skills-framework subsection; 42→43). Both dates → 2026-07-01; neither lifted past existing confidence ceilings (single-source, promotional-podcast interview, unquantified beyond one self-reported 94%-PR-rejection figure).
- No entities promoted: Jesse Vincent, CodeRabbit, and host Hrik are single-source mentions (Dangling) per the second-source promotion rule — Jesse Vincent/Superpowers flagged as a strong future promotion candidate given direct relevance to this repo's own tooling.

## [2026-07-01] ingest | Microsoft WorkLab *"Job Titles Don't Matter in 2026 (Here's What Does)"* — Aneesh Raman (LinkedIn)

Acquired via the youtube-transcript-skill (`--json` route; the raw fetch captured the transcript panel in two overlapping scroll passes, a known quirk — 704 raw segments deduplicated by (timestamp, normalized text) to 352 unique segments before landing) and processed the same session. A ~38-minute Microsoft **WorkLab** podcast episode (host Molly Wood) interviewing **Aneesh Raman**, LinkedIn's Chief Economic Opportunity Officer and co-author of *Open to Work: How to Get Ahead in the Age of AI*. A **pro-human, agency-centered** framing at three altitudes: individual ("onlyness" — identity as a unique combination of curiosities/capabilities beyond job titles), company (the electricity-bolt-on-without-redesign historical analogy; three organizational shifts — lead by design not command, capability not category, develop people not tasks), and society (net-employment optimism via Jensen Huang's "won't run out of jobs unless we run out of ideas"; a labor-market-opacity + Global-South-equity argument via Brad Smith's electricity-access cautionary tale). Names the **5 C's** (creativity, curiosity, courage, compassion, communication) as trainable durable-skills.

- New source page: [[2026-06-29-raman-wood-worklab-job-titles-dont-matter-2026]] (`kind: video`; raw at `raw/videos/job-titles-dont-matter-in-2026-heres-what-does.md`; human-curated/manual English captions).
- W&W tags (4): `digital-sensing/digital-mindset-crafting`, `digital-transforming/redesigning-internal-structures`, `digital-transforming/improving-digital-maturity`, `strategic-renewal/organizational-culture`.
- Neighbour-source scan (4 candidates surfaced) → 4 typed `supports` edges: [[2026-05-02-schoening-lennys-podcast-cultivating-agency-ai-era]] (agency-over-job-titles, independent near-identical framing), [[2026-06-12-argenti-hbr-thrive-alongside-ai-mindset-not-skillset]] (mindset-not-skillset, same individual-response move), [[2026-06-18-dumra-mit-smr-dbs-everyone-an-innovator]] (operator-scale worked example of the org-redesign thesis), [[2026-05-13-storoni-hbr-ideacast-redefining-efficiency-age-ai]] (shared rejection of efficiency-only metrics). ~55 other sources shared at least one W&W cell or ai-employment-effects citation; considered and deferred as not defensibly distinct (see [§Ingest step 5](../CLAUDE.md#process)).
- Concept updates: [[concepts/warner-wager-process-model]] (new individual-and-leadership-agency subsection; 5→6), [[concepts/ai-employment-effects]] (new onlyness/net-employment-optimism subsection; 55→56), [[concepts/durable-skills]] (new 5-C's subsection; 34→35), [[concepts/micro-productivity-trap]] (new electricity-precedent subsection; 35→36). All four dates → 2026-07-01; none lifted past their existing confidence ceilings (single-source, promotional-book popularisation per Lifecycle rules).
- Entity update: [[Microsoft]] (2→3 sources; confidence 0.8→0.85; new `published-by` edge for the WorkLab channel, distinct from the *Microsoft Visual Studio* and Nadella/*Possible* vantages already held).
- No entities promoted: Aneesh Raman, Molly Wood, Ryan Roslansky, Kevin Scott, Brad Smith, Conor Grennan, Jensen Huang are single-source mentions (Dangling) per the second-source promotion rule. LinkedIn is a candidate entity (Raman's employer, a Microsoft subsidiary since 2016) — deferred to a second LinkedIn-anchored source.
- One deliberate non-edge: considered a typed relationship to [[concepts/expert-generalist]] ("onlyness" vs the Expert Generalist trait-set) but declined — that page enforces a strict vendor-propagation-cap discipline requiring sources to use its specific vocabulary before counting toward confidence; noted as a body cross-reference on the new source page instead, without touching expert-generalist.md's frontmatter.

## [2026-06-27] ingest | RaboResearch *"Hoe AI de sectoren IT en zakelijke dienstverlening verandert"* (Rabobank, 25 Jun 2026) — via Zotero

Acquired from the Zotero `ai-wiki` collection (1 new item; the other 11 already in `raw/`) and processed the same session. A **RaboResearch (Rabobank)** Dutch-market sector study applying the **ILO occupational-exposure method** (Gmyrek et al. 2025, ILO WP 140) to **CBS** occupation data to estimate AI's automation and productivity potential for the **IT** and **zakelijke dienstverlening** sectors. The wiki's **first Dutch national/sectoral** source on AI labour + productivity. The Zotero text extract was complete prose but **figure-free**; because the 13 figures carry the quantitative payload, they were fetched from the live rabobank.nl page (bot-blocked to plain fetchers; rendered in a real browser, the CDN serves the chart images) and read visually — figs 4, 5a/5b, 6, 7a, 9a/9b transcribed in full.

- New source page: [[2026-06-25-raboresearch-ai-it-zakelijke-dienstverlening]] (`kind: article`; `zotero_item_key: 7A3RK8MR`; raw at `raw/articles/hoe-ai-de-sectoren-it-en-zakelijke-dienstverlening-verandert.md`). `date_published` recovered as 2026-06-25 from the body (stub had it empty); authors split from the stub's single jammed string.
- Headline data: **86% of IT tasks / 64% of business-services tasks** (hoog) automatiseringspotentieel; per-function decomposition; an ~18-study productivity spread (**+73% → +1.4% firm-level / Atlassian 96% no ROI**); NL ahead of EU average but below EU leader; per-occupation rankings (routine-cognitive high, physical low); three enabling conditions (strategic ownership · data/IT/process base · governance/trust).
- W&W tags (4): `digital-sensing/digital-scenario-planning`, `digital-sensing/digital-mindset-crafting`, `digital-transforming/improving-digital-maturity`, `contextual/external-triggers`. Governance/trust folded into improving-digital-maturity + [[concepts/responsible-ai]] (no dedicated governance cell).
- Neighbour-source scan (≥3 candidates surfaced) → 3 typed `supports` edges: [[2026-04-28-brynjolfsson-canaries-coal-mine]] (occupational-exposure corroboration of the entry-level/compositional shift), [[2026-04-03-bcg-emerson-kropp-ai-will-reshape-more-jobs-than-it-replaces]] (reshape≫replace at sector altitude), [[2026-03-05-massenkoff-mccrory-anthropic-labor-market-impacts-ai]] (method-cousin occupational-exposure measure). MGI and Evans considered and deferred (already connected via the concept page).
- Concept updates: [[concepts/ai-employment-effects]] (new Netherlands-sectoral subsection; 54→55), [[concepts/micro-productivity-trap]] (new task-vs-firm productivity-spread subsection from fig 6; 34→35), [[concepts/enterprise-ai-adoption]] (new Dutch national-adoption datapoint; 78→79). All three dates → 2026-06-27, confidence held at 0.95 ceiling.
- No entities promoted: all four authors + RaboResearch/Rabobank + CBS + ILO/Gmyrek are single-source mentions (Dangling) per the second-source promotion rule.
- One data caveat carried into the pages: fig 6 plots METR at **+18%** where METR's own July-2025 RCT found experienced devs ~19% *slower* — sign-discrepancy flagged, not taken at face value.
...
**Acquire.** Transcripts fetched via the [`youtube-transcript-skill`](../../.claude/skills/youtube-transcript-skill/SKILL.md) (Playwright; `playwright install chromium` was re-run first). The ~48-min Mozilla talk hit the known long-format **"transcript panel did not render"** failure (GH #2) even at 60s; recovered via a **`yt-dlp` auto-sub fallback** (en, de-duplicated). Raw files: `raw/videos/how-mozilla-uses-claude-mythos-to-find-firefox-bugs.md`, `raw/videos/how-to-get-your-first-10-customers.md`, `raw/videos/satya-nadella-ai-is-the-future-of-the-firm.md`.

**Process.**
- [[2026-06-22-grinstead-how-i-ai-mozilla-firefox-agentic-security-harness]] — Mozilla's production agentic security-bug harness (LLM-judge → analyzer → fuzzing/AddressSanitizer → verifier subagent → patch), ~500 Firefox fixes/month, **~50/50 model-vs-harness** split. `supports` Vo (same-channel goal-loop), Lopopolo, Chatterjee, AEI returns-to-expertise; **`contradicts`** Kilpatrick (model-eats-the-harness). 3 W&W tags.
- [[2026-06-22-yc-kolysh-how-to-get-your-first-10-customers]] — YC Startup-School GTM-tactics playbook (warm network → do-things-that-don't-scale → tools at 10–20). Only lightly AI → **no W&W tags**. `supports` Dinakaran/Luminai + Glasgow/Campfire.
- [[2026-06-05-nadella-hoffman-possible-ai-future-of-the-firm]] — platform-CEO worldview: **token capital**, *AI-is-the-future-of-the-firm*, the hill-climbing machine, *don't-use-frontier-models-for-non-frontier-problems*, Agent 365, Foundry asserts, the agentic development environment, cognitive coverage, social permission. 8 W&W cells (roles override → C-suite). `supports` Ramaswamy, Dumra, AWS-data-strategy, Argenti, Beutler.

**Concepts updated** (source_count, last_confirmed/accessed_at → 2026-06-23, new body sections): [[concepts/agent-harness|agent-harness]] (66→68), [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (76→77), [[concepts/automation-vs-augmentation|automation-vs-augmentation]] (48→50), [[concepts/responsible-ai|responsible-ai]] (15→16), [[concepts/durable-skills|durable-skills]] (31→32), [[concepts/agentic-engineering|agentic-engineering]] (40→41), [[concepts/ai-employment-effects|ai-employment-effects]] (50→51), [[concepts/ai-agents|ai-agents]] (20→21). **Entities updated**: [[How-I-AI]] 2→3, [[Claire Vo]] 2→3, [[Anthropic]] 21→22, [[Microsoft]] 1→2, [[GitHub]] 1→2, [[OpenAI]] 18→19, [[Y Combinator]] 13→14. No new entity pages (new people — Nadella, Hoffman, Grinstead, Kolysh — left Dangling per the second-source promotion rule). qmd re-embedded (`update` then `embed`).

## [2026-06-22] ingest | Vo *How I AI* — [[2026-06-17-vo-how-i-ai-ai-agent-loops-claude-code-codex|How to write AI agent loops in Claude Code and Codex]]

Acquired + processed in one session. YouTube ingest (https://youtu.be/JoXbk2fm7jM) — the Playwright engagement-panel path hit the known "transcript panel did not render" long-talk failure mode, so the transcript was fetched via the `youtube-transcript-api` fallback (`timedtext` endpoint, 747 ASR segments) and ASR-cleaned (OpenClaw, ChatPRD, Codex, Claude Cowork, chat-smoke CLI; sponsor-read music markers stripped). Host identity (Claire Vo) + proper nouns confirmed against the video description's link block. Raw at `raw/videos/how-to-write-ai-agent-loops-claude-code-codex.md`.

Source page: the **product-leader / non-engineer altitude** on the self-prompting **loop** — *"a loop is just an automated prompt."* Contributes a clean **loop-trigger taxonomy** (heartbeat / cron / hook / **goal**) and the **goal loop as a named first-class primitive** in Claude Code (`/goal`, routines) + Codex (automations), the **five-things-an-effective-loop-needs** checklist (work trees, skills, plugins/connectors, subagents, state — crediting [[Addy Osmani]]), and the **"onboarding an employee"** design frame. No W&W `dynamic_capabilities:` tags (practitioner engineering, outside the lens — consistent with the Lopopolo/Osmani neighbours).

Neighbour-source scan (≥3 candidates surfaced): edges `supports` → [[2026-06-19-lopopolo-ai-native-devcon-harness-engineering]] (engineer-altitude companion), [[2026-05-15-osmani-agent-harness-engineering]] (loop/Ralph-loop article Vo cites), [[2025-11-26-anthropic-effective-harnesses-long-running-agents]] (the goal loop vendor-side). Skipped: Lopopolo/Codex blog (transitive) and harness-is-all-you-need (adjacent, no claim-level link).

**Promoted two entities** — the dangling-author lint flagged **How I AI** at 2 source mentions (the other being [[2025-12-01-marily-nika-pms-who-use-ai-will-replace-those-who-dont|the Marily Nika episode]], which had explicitly deferred promotion to a second source). Created [[How-I-AI]] (kind: venue) and [[Claire Vo]] (kind: person, host + ChatPRD founder); updated the Marily Nika source's dangling notes to point at the new pages. Lint now clean.

**Cross-reference repair**: also created [[Claude Code]] (kind: product, part-of [[Anthropic]]) — the wiki had **12 pre-existing `[[Claude Code]]` wikilinks across 9 files with no target page** (all latent broken links; ~45 sources mention Claude Code in prose). Creating the entity resolves all of them at once plus the 3 new links this ingest added. `aliases:` includes *Claude Cowork*. Noted Codex as the parallel promotion candidate (1 dangling `[[Codex]]` link).

Touched: [[concepts/agent-harness|agent-harness]] (65→66; new §loop-trigger taxonomy; last_confirmed 2026-06-22), [[concepts/agentic-engineering|agentic-engineering]] (39→40; new §loops-as-designing-jobs), [[Addy Osmani]] (source_count 2→3; added "mentioned in" for the cited loop-engineering article), [[2025-12-01-marily-nika-pms-who-use-ai-will-replace-those-who-dont|Marily Nika source]] (dangling notes → promoted), `index.md` (new Sources entry + 2 entities). Still dangling (single-source, deferred): **OpenClaw**, **ChatPRD**, **Marily Nika**.

## [2026-06-20] ingest | Lopopolo *Harness Engineering* talk via zotero-acquire — [[2026-06-19-lopopolo-ai-native-devcon-harness-engineering|How to Build Software When Humans Steer and Agents Execute]]

Acquired the one new `ai-wiki` Zotero item (key UQRQ5D5Z, `videoRecording`) — auto-delegated to the [[youtube-transcript-skill]]; ASR transcript (no chapters; doubled, duplicate disregarded). **[[Ryan Lopopolo]]** (OpenAI; **coiner of "harness engineering"**) at **AI Native DevCon** — the conference-talk companion to his [[2026-02-11-lopopolo-codex-harness-engineering|Codex blog case study]]. Supplies the **named definition** (*"making context around what it means to do a good job legible, and just-in-time surfacing it to the agent"*), the **three foundational limits** (human time / attention-sums-to-one / context window), the **three-phase context-delivery model** (ground → just-in-time-steer-via-tool-calls → LM-as-judge review), **shift-right-not-left** + *"never give the same review feedback twice"*, **all-code-is-prompts → prune latent space**, review personas + coarse structural guardrails, and the **group-tech-lead operating mode** (vibe coding as enabler). No W&W tags (engineering-practice talk, consistent with the prior Lopopolo source). **Promoted [[Ryan Lopopolo]] to an entity** (2nd substantive source per the promotion rule; updated the Codex-blog source's dangling line). **Neighbour-scan**: 4 `supports` edges ([[2026-02-11-lopopolo-codex-harness-engineering|Codex blog]] same-author, [[2026-05-04-rethinking-agents-harness-is-all-you-need|harness-is-all-you-need]], [[2026-05-08-bratanic-unified-agentic-memory-hooks|Bratanic dream-phase]], [[2026-05-06-bockeler-engineering-of-ai-agents-context-harnessing-autonomy|Böckeler]]); Kilpatrick tension noted on the agent-harness Debates section, not edged. **Touched** (≈9 files): [[OpenAI]] (17→18), 5 concepts ([[concepts/agent-harness|agent-harness]] 64→65, [[concepts/agentic-engineering|agentic-engineering]] 38→39, [[concepts/vibe-coding|vibe-coding]] 21→22, [[concepts/automation-vs-augmentation|automation-vs-augmentation]] 47→48, [[concepts/software-3.0|software-3.0]] 7→8), `index.md`. Dangling first-mentions: AI Native Dev / AI Native DevCon, Artichoke (`artichoke-rand-mt`).

## [2026-06-20] ingest | Snowflake CEO on the McKinsey Podcast via zotero-acquire — [[2026-06-18-ramaswamy-mckinsey-every-company-software-company|Ramaswamy: AI Is Turning Every Company Into a Software Company]]

Acquired the one new `ai-wiki` Zotero item (key PMSZEARN, `videoRecording`) — auto-delegated to the [[youtube-transcript-skill]]; ASR transcript, came back doubled (duplicate intro disregarded). **Sridhar Ramaswamy** (CEO, **Snowflake**) interviewed by **Eric Kutcher** (McKinsey North America Chair) on the **McKinsey Podcast** — the wiki's **first McKinsey-Podcast-channel ingest**. CEO-vantage read on the *industrialization of intelligence*: cost-of-software collapse; **"uber programmers" 50–100×**; critic + security-critic agents; conversational interface as the new natural interface; **consumption pricing** + *not every token has the same value* + open-source-hosted-alongside-frontier; **two ROI surefire hits** (software engineering + support, built in 6 weeks on Cortex Code); **business-transformation-not-technology** + **viral adoption** ("Coco" + Snowhouse) + **redeploy-don't-cut**; protect-your-career; malleability/first-principles-generalism as hiring criteria. **8 W&W cells** (roles override → `ceo, coo, cto, cdo, transformation-lead`). **Neighbour-scan**: 4 `supports` edges ([[2026-05-20-tan-hu-stanford-cs153-ai-native-company-1000x-engineer|Tan & Hu]], [[2026-06-17-ng-langchain-interrupt-future-of-ai-agents|Ng]], [[2026-05-21-bender-google-io-software-engineering-tipping-point|Bender]], [[2026-05-13-jha-emergent-democratizing-app-building-with-claude|Jha]]); Hu/Masad/Chamath/Warren surfaced but not edged (no specific-claim engagement). **Touched** (≈11 files): [[McKinsey & Company]] (9→10), 8 concepts ([[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] 75→76, [[concepts/agentic-engineering|agentic-engineering]] 37→38, [[concepts/vibe-coding|vibe-coding]] 20→21, [[concepts/software-3.0|software-3.0]] 6→7 conf 0.85→0.88, [[concepts/automation-vs-augmentation|automation-vs-augmentation]] 46→47, [[concepts/durable-skills|durable-skills]] 30→31, [[concepts/ai-employment-effects|ai-employment-effects]] 49→50, [[concepts/foundation-models|foundation-models]] 17→18), `index.md`. Dangling first-mentions: Sridhar Ramaswamy, Snowflake (+ Cortex Code/Coco, Snowwork, Snowhouse), Eric Kutcher, Neeva.

## [2026-06-20] ingest | Three June-2026 videos via zotero-acquire — Ng (LangChain Interrupt) + Mollick (A Bit of Optimism) + Chou (YC Lightcone)

Acquired three `videoRecording` items from the `ai-wiki` Zotero collection (keys JUUQZGWX / CWRX4K3S / VRWYL2YC); all three auto-delegated to the [[youtube-transcript-skill]]. The Y Combinator item's first fetch returned no captions (landed an abstract-only stub); re-fetched manually and the Zotero provenance was merged back. All three transcripts came back **doubled**; the duplicate tail was disregarded. **Title mismatch handled** (pre-flight Check 2): the Zotero title *"Why Domain Experts Are Winning Right Now"* (VRWYL2YC) ≠ the actual YouTube title *"The Age Of The 40-Year-Old Solo Founder Is Here"* — wiki page slugged for content, raw filename preserved.

A tight thematic cluster on **returns to expertise / taste in the agentic era**, landing squarely in the wiki's harness + durable-skills neighbourhood:

- **[[2026-06-17-ng-langchain-interrupt-future-of-ai-agents|Ng / LangChain Interrupt '26]]** (host [[Harrison Chase]]) — PM-bottleneck-becomes-everything-bottleneck; small high-context generalist teams; building-blocks/LEGO; **enterprise vantage** (AI Aspire): bottom-up "thousand flowers" not paying off vs top-down workflow redesign (10-minute-loan); optionality (≤1-yr contracts, open-weight, LangSmith); the coming **unstructured-data rearchitecture**. 8 W&W cells. 5 `supports` (returns-to-expertise, Dumra/DBS, Beutler, Allen/AWS, Kilpatrick).
- **[[2026-06-16-mollick-simon-sinek-ai-skills-experience-edge|Mollick / A Bit of Optimism]]** — experience beats "AI-native" youth (BCG study); taste as differentiator; apprenticeship-broke; model/apps/harnesses framing; GDPval 48%→84%; doubling-down-on-human. 4 W&W cells. 6 `supports` (returns-to-expertise, GDPval, Kilpatrick, AEI-Q4 deskilling, Argenti, Chou).
- **[[2026-06-19-chou-yc-lightcone-40-year-old-solo-founder|Chou / YC Lightcone]]** — expertise as the lever on boundless model intelligence; the 40-yo-solo-founder/taste thesis; Ploy as an "anti-slop" opinionated vibe-coding platform + purpose-built harness; agents-as-customers (AEO/GEO, CLI-with-skills over MCP). 5 W&W cells. 4 `supports` (returns-to-expertise, harness-is-all-you-need durable-pole, Mollick, Ng).

**Promoted** [[Simon Sinek]] to an entity (second source — also the 2018 NYT *Infinite Game* keynote subject; updated that source's dangling line). **Entity bumps**: [[Andrew Ng]] (5→6), [[Ethan Mollick]] (1→2), [[Harrison Chase]] (2→3), [[LangChain]] (6→7), [[DeepLearningAI]] (4→5), [[Boston Consulting Group]] (4→5). **Concepts touched** (≈11): [[concepts/durable-skills|durable-skills]] (27→30), [[concepts/agent-harness|agent-harness]] (62→64), [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (73→75), [[concepts/automation-vs-augmentation|automation-vs-augmentation]] (44→46), [[concepts/ai-employment-effects|ai-employment-effects]] (47→49), [[concepts/jagged-frontier|jagged-frontier]] (13→14), [[concepts/ai-deskilling|ai-deskilling]] (11→12), [[concepts/vibe-coding|vibe-coding]] (19→20), [[concepts/agentic-engineering|agentic-engineering]] (36→37), [[concepts/software-3.0|software-3.0]] (5→6, conf 0.82→0.85), [[concepts/dynamic-capabilities|dynamic-capabilities]] (10→11). Plus `index.md`. Dangling first-mentions: Y Combinator (channel), Bryant Chou, Ploy/Webflow, Chris Tann, Rohit Prasad, AI Aspire/AI Fund, Marvin Minsky, Parker Conrad/Rippling.

## [2026-06-19] ingest | MIT SMR *Leaders at All Levels* Ep. 9 — [[2026-06-18-dumra-mit-smr-dbs-everyone-an-innovator|Dumra: How DBS Bank Makes Everyone an Innovator]]

Acquired via the [[youtube-transcript-skill]] (Playwright/DOM route; the fetch returned the transcript **doubled** — 594 segments repeated — and the duplicate tail was removed at ingest) and processed in the same session. **[[Bidyut Dumra]]** (Group Head of Innovation and Future of Work, **[[DBS Bank]]**) on how DBS made innovation a company-wide KPI-backed system across 39,000 employees — the wiki's **first *Leaders at All Levels* ingest** (a distinct MIT SMR show from *Me, Myself, and AI*) and its **richest single operator-altitude [[concepts/dynamic-capabilities|dynamic-capabilities]] case**. Headline mechanisms: **GANDALF** ("be the D in GANDALF" — sensing via competitor re-framing); **Managing Through Journeys** (operating model reoriented horizontally around customer *intent*); **mini-CEOs + QPR dynamic funding + slush fund**; the **Innovation Pyramid** (big bets / Horizon 3 / journeys / entrepreneurs, each a KPI); the **Playbook Model** (20%-of-scorecard transformation KPI to every employee; 4D framework; *"250 journeys or 10,000 agents"*); **information gap → action gap** (*"the wrong way is to do nothing"*). **11 W&W cells** across all five buckets (roles override → `ceo, chro, cdo, transformation-lead, innovation-lab-lead`). **Neighbour-source scan** surfaced 5 `supports` edges: [[2026-05-14-price-dfi-retail-asia-reinventing-how-it-sells|Price/DFI]], [[2026-05-24-erginbilgic-bloomberg-leaders-rolls-royce-turnaround-playbook|Erginbilgiç/Rolls-Royce]], [[2026-06-02-architecting-ai-native-organizations-redesign-work-at-scale-joe-beutler|Beutler]], [[2026-05-21-allen-aws-london-exec-forum-agentic-team-structures|Allen/AWS]] (9 shared cells), [[2025-05-06-jassy-amazon-agility-ai-strategy-changing-role-of-managers|Jassy/Amazon]]. **Created** [[Bidyut Dumra]] + [[DBS Bank]] entities. **Touched** (≈8 files): [[MIT Sloan Management Review]] (6→7 sources), [[concepts/dynamic-capabilities|dynamic-capabilities]] (9→10), [[concepts/warner-wager-process-model|warner-wager-process-model]] (3→4), [[concepts/enterprise-ai-adoption|enterprise-ai-adoption]] (72→73), [[concepts/micro-productivity-trap|micro-productivity-trap]] (33→34), [[syntheses/organizational-frameworks-for-ai-adoption|organizational-frameworks]] synthesis (13→14; DBS = first lived multi-framework operator case, partial answer to its open question #4), `index.md`. Dangling first-mentions: Kate Isaacs, Michael (co-host), Cathay Pacific.

## [2026-06-18] ingest | Google Cloud *Open Knowledge Format* (OKF v0.1) via zotero-acquire — [[2026-06-12-mcveety-hormati-google-cloud-open-knowledge-format|McVeety & Hormati]]

Acquired from the `ai-wiki` Zotero collection (webpage item, Zotero-extracted text; key `XRZZ9MTA`; `date_published` recovered from the post body). **OKF v0.1 is the first major-vendor formalization of the [[llm-wiki|LLM Wiki]] pattern** into an open spec — a directory of markdown concept-files + YAML frontmatter (only `type` required), optional `index.md` / `log.md`, markdown-cross-links-as-graph; it explicitly cites Karpathy's gist and names the **CLAUDE.md / AGENTS.md family** as prior instances. The **most self-referential source in the wiki** — this repo already uses every OKF convention. Edges: `instance-of` [[llm-wiki]], `supports` [[2026-04-16-raju-rag-isnt-dead-karpathys-llm-wiki-explained|Raju]] + [[2026-04-27-liu-rag-llm-wiki-or-gbrain-how-your-agent-remembers|Liu]], `published-by` [[Google]]. 1 W&W tag (`contextual/internal-enablers`).

Updated [[concepts/llm-wiki|llm-wiki]] (7→8 sources, confidence 0.91→0.93; new **§OKF — the pattern becomes an open standard**, standardization vantage added to the source list, and the *enterprise-implementations* open question marked partially-resolved) and [[Google]] (10→11 sources; OKF logged as a second *vendor-format-as-open-standard* instance alongside Bfloat16). Dangling first-mentions: Sam McVeety, Amir Hormati. Touches ~5 files. Reversible.

## [2026-06-18] acquire | Zotero batch `ai-wiki` (2 new) + backfill log/index for [[2026-06-16-anthropic-economic-index-agentic-coding-returns-to-expertise|Agentic coding & returns to expertise]]

Ran [`zotero-acquire`](../../.claude/skills/zotero-acquire/SKILL.md) against the `ai-wiki` collection: 5 items, 3 skipped (`zotero_item_key` dedup), 2 new — OKF (separate ingest entry above) and *Agentic coding and persistent returns to expertise* (Hitzig, Massenkoff, Lyubich, Heller, McCrory; [[Anthropic]] Economic Index). The agentic-coding source page was created in the **2026-06-17 session but never logged or indexed**; this entry backfills both. **Reconciliation**: Zotero supplied dedup key `N6KSNEQM` + a descriptive PDF filename; corrected the Zotero `journalArticle` mis-type → `report` (routing `papers/` → `reports/`); consolidated the duplicate raw PDF (removed the non-descriptive `CCEconReport-G.pdf`); added `zotero_item_key` and fixed the `raw:` pointer on the existing wiki page. No wiki-body changes — the prior page is complete (author entities [[Eva Lyubich]] / [[Ryan Heller]] and concept touches were done in the prior session). Reversible.

## [2026-06-16] ingest | Two HBR articles via zotero-acquire — [[2026-07-01-mcgrath-power-of-strategic-centering|McGrath *Strategic Centering*]] + [[2026-06-10-anicich-brouwers-why-employees-arent-transparent-ai-usage|Anicich & Brouwers *Why Employees Aren't Transparent About AI*]]

Acquired 2 new `ai-wiki` Zotero items (PDFs → `pdftotext`; the Kilpatrick video correctly skipped by `zotero_item_key` dedup) and processed both as a supervised batch.

**Source 1 — [[2026-07-01-mcgrath-power-of-strategic-centering|McGrath, *The Power of Strategic Centering*]]** (HBR Jul–Aug 2026). New concept **[[strategic-centering]]**: choose one organizing principle ("What are we really about?") to keep strategy coherent as value **dematerializes** (~90% intangible; Carlota Perez turning point) and the classical anchors (Porter / RBV / Blue Ocean) lose force. Five centers — mission / customer / technology / national-ecosystem / friction-erasure. Edges: `instance-of` [[strategy]] + [[strategic-centering]], **`supports`** [[2022-02-23-oberholzer-gee-hbr-what-is-strategy-value-stick|Oberholzer-Gee value stick]] (complementary modern-strategy reframing). Updated [[strategy]] (6→7; centering as a third lens). 3 W&W tags.

**Source 2 — [[2026-06-10-anicich-brouwers-why-employees-arent-transparent-ai-usage|Anicich & Brouwers, *Why Employees Aren't Transparent About Their AI Usage*]]** (HBR, 10 Jun 2026). New concept **[[ai-knowledge-hiding]]** (the "suppression of solutions"): employees hide discovered AI workflows; driver is **organizational trust** via **psychological safety**, not governance/tooling (own survey n=604: 30.3% withhold; 47% vs 14% by trust quartile; Stanford 51-deployment → 77% of hard challenges non-technical). Three disclosure costs (reputational / workload / replaceability). Edges: `instance-of` [[enterprise-ai-adoption]], `supports` [[micro-productivity-trap]]; source-level **`supports`** [[2026-04-28-mit-sloan-ai-maturity]] + [[2025-12-22-randell-gousset-microsoft-agentic-devops-in-real-life|the "trust gap"]]. Updated [[enterprise-ai-adoption]] (71→72; disclosure-culture section) + [[micro-productivity-trap]] (32→33; individual efficiency-tax).

**Touches (~10 files)**: 2 new sources + 2 new concepts + 3 concept updates ([[strategy]], [[enterprise-ai-adoption]], [[micro-productivity-trap]]) + index + log. No entity promotions (all authors first-mention → dangling: Rita McGrath, Carlota Perez, Eric Anicich, Jeslyn Brouwers, Amy Edmondson). Reversible.

## [2026-06-15] ingest | [[2026-06-11-kilpatrick-sequoia-model-eats-the-harness|Kilpatrick *Why the Model Eats the Harness*]] (Sequoia / Google DeepMind, 11 Jun 2026) — first source via the zotero-acquire channel; the "model eats the harness" counter-thesis

**First end-to-end ingest through the new [`zotero-acquire`](../.claude/skills/zotero-acquire/SKILL.md) channel** (Zotero item `R4UDMW3G`, collection `ai-wiki`). The acquired stub was a YouTube video with `fulltext_source: none` (Zotero had only the abstract), so the transcript was re-acquired via [`youtube-transcript-skill`](../.claude/skills/youtube-transcript-skill/SKILL.md) (~51 min, 1000 ASR lines, 16 chapters) and the Zotero provenance merged into the raw file's frontmatter — establishing the **Zotero→YouTube delegation pattern** for `videoRecording` items with a YouTube URL.

**Substance**: [[Logan Kilpatrick]]'s **"model eats the harness"** thesis — scaffolding runs ahead of the model, then the model digests it and the alpha moves elsewhere; harness alpha is **transient (~12 mo)**, not the durable moat. Recorded as a new **`contradicts`** edge to [[2026-05-04-rethinking-agents-harness-is-all-you-need|"harness is all you need"]] (**shared subtraction principle, opposite strategic conclusion**) and a **`supports`** edge to the DeepMind [[2026-03-05-lou-deepmind-autoharness-code-harness-synthesis|AutoHarness]] paper (model *synthesises* the harness). Also: [[Antigravity]] as Google's single harness (ex-Windsurf) rebasing search/Gemini-app/Cloud/AI-Studio; **jagged/narrow superintelligence**; **outcomes-over-eyeballs**; [[Omni]] (one any-to-any model replacing ~8); **350k Android apps in AI Studio in a week**.

**Touches (~11 files)**: 1 new source + **5 entity promotions** ([[Logan Kilpatrick]], [[Google DeepMind]], [[Sequoia Capital]], [[Antigravity]], [[Omni]]) + **2 concepts** — [[agent-harness]] (61→62; new *durable-vs-transient harness alpha* Debate bullet + harness-bench) and [[vibe-coding]] (18→19; AI Studio 350k-apps / personal-software row) + index (Sources) + log. 5 W&W tags on the source. Dangling: Sonya Huang, Demis Hassabis, Tulsi Doshi, Sundar Pichai, Windsurf. Reversible.

## [2026-06-15] refactor | New acquire channel: `zotero-acquire` skill + CLAUDE.md Zotero-stub contract

Landed the [`zotero-acquire`](../.claude/skills/zotero-acquire/SKILL.md) skill — the canonical Acquire-time skill for a **local Zotero 7** library, paralleling `youtube-transcript-skill` for videos. Reads the local Zotero HTTP API via `pyzotero` (`Zotero(0,"user",local=True)`, no cloud/key), scoped to the dedicated **`ai-wiki`** collection; copies each item's PDF into `raw/<type>/<slug>.pdf` (gitignored, co-located by type), converts via the normal pipeline (`marker`→`markitdown`→`pdftotext`; only `pdftotext` installed here), and writes a `raw/<type>/<slug>.md` stub carrying the Acquire→Process frontmatter contract (`zotero_item_key` dedup, `item_type` routing hint, `fulltext_source` honest-scoping). **Verified** end-to-end against the live library (dry-run + live write + dedup-skip on the `State of AI` collection). **CLAUDE.md changes**: new §Verifying-sources *Pre-flight check (Zotero stubs)* subsection + field-mapping table; §Acquire names the skill and the `acquire | zotero-batch …` log op; fixed the `raw/assets/` PDF-location drift → co-locate-by-type. Design spec: `docs/superpowers/specs/2026-06-15-zotero-acquire-design.md`. **Not yet done**: the `ai-wiki` collection is still empty — the ≥3-items end-to-end Process run (Definition-of-done) is pending the user populating it. No wiki content pages touched.

## [2026-06-15] refactor | Reclassify Zotero-stack integration from wiki thread → `inspiration/` notes

The page filed earlier today as `wiki/threads/zotero-stack-integratie.md` was reclassified as a plain research/decision note: it is build-planning for the wiki's own tooling (pairing with the May-2026 PRD), not a knowledge-investigation thread. **Moved** `wiki/threads/zotero-stack-integratie.md` → `inspiration/2026-06-15-zotero-stack-integratie.md` (git mv); dropped the thread-schema frontmatter (`type: thread` / `status: open` / `opened:`) for an `inspiration/`-style header (`title` / `date` / `status: notes`); rewrote sibling PRD references and the "How this thread should resolve" → "Definition of done" framing. **Touches**: file move + index (removed from Threads section) + this log. The 2026-06-15 `query` entry below is retained for history (its wikilinks neutralised to plain text since the target left `wiki/`). Reversible: `git mv` back and restore the index bullet.

## [2026-06-15] ingest | [[2026-04-03-bcg-emerson-kropp-ai-will-reshape-more-jobs-than-it-replaces|BCG *AI Will Reshape More Jobs Than It Replaces*]] (Emerson, Kropp et al., 3 Apr 2026) — six AI Labor Disruption Segments; method-independent corroboration of the Anthropic observed-exposure report

22-page BCG Henderson Institute microeconomic labor-impact report (full read). **Reshape ≫ replace**: 50–55% of US jobs reshaped in 2–3 yrs, 10–15% eliminated in 4–5 yrs; 43% are ≥40% automatable. **Six AI Labor Disruption Segments** = substitution-vs-augmentation × **demand expandability** (Jevons): Amplified 5% / Rebalanced 14% / Divergent 12% / Substituted 12% / Enabled 23% / Limited-Exposure 34%. Four CEO directives (embed workforce strategy / redesign-not-cost / continuous upskilling / shape the narrative). **Method-independent convergence** with the [[2026-03-05-massenkoff-mccrory-anthropic-labor-market-impacts-ai|Anthropic observed-exposure report]] — different methods, same headline — recorded as a `supports` edge. `kind: report`; 5 W&W tags.

**Touches (~14 files)**: 1 new source + **3 entity promotions** ([[Matthew Kropp]], [[Julie Bedard]], [[Megan Hsu]] — 2nd-source rule, all also author the [[2026-05-06-kropp-bcg-hbr-dont-treat-ai-agents-like-employees|BCG agents-as-employees piece]]; surfaced/cleared by dangling-author lint) + [[Lisa Krayer]] (2→3) + [[Boston Consulting Group]] (3→4) + **5 concepts**: [[ai-employment-effects]] (45→46; six-segment taxonomy), [[automation-vs-augmentation]] (43→44; §23 — the demand-expandability/Jevons second axis), [[durable-skills]] (26→27; rising skill thresholds + AI-fluency-as-tenure-complement), [[micro-productivity-trap]] (31→32; redesign-not-cost CEO directive), [[enterprise-ai-adoption]] (70→71; adoption-lag + forward-deployed-engineers + 4 CEO directives) + index + log. Typed edges: `published-by` BCG, `supports` Kropp-BCG + Massenkoff-McCrory + Brynjolfsson. Dangling: Greg Emerson (lead, BCG MD), Romanov, Sanchez Boedo, Mohnot. Reversible.

## [2026-06-15] refactor | Normalize 10 concept "Debates / contradictions" headings → canonical "Debates and supersession"

Schema-alignment sweep (CLAUDE.md mandates the exact heading; `scripts/quality-score.mjs` keys on the literal string). Heading-only rename on agent-harness, ai-agents, ai-benchmarks, ai-employment-effects, automation-vs-augmentation, foundation-models, generative-ai, industry-4-0, lean-4-0, responsible-ai; quality_score recomputed (agent-harness 0.77→0.97, ai-employment-effects 0.77→0.97, automation-vs-augmentation 0.73→0.93, responsible-ai 0.69→0.89, ai-agents 0.67→0.87). **Corpus now has zero concepts below 0.65.** Content unchanged.

## [2026-06-15] query | Filed Zotero-stack integration thread — local-Zotero → wiki integration research + decisions

> Superseded by the 2026-06-15 `refactor` entry above: this was reclassified to `inspiration/2026-06-15-zotero-stack-integratie.md`; wikilinks below are neutralised to plain text.

Researched connecting the wiki to a **local** Zotero instance so that adding a source stops meaning hand-copying and converting files into `raw/`. Key finding that re-prices the May-2026 PRD (`inspiration/2026-05-14-prd-zotero-stack-integratie.md`, which assumed the cloud Web API + the `ai-consultant` GCP stack): **Zotero 7 ships a read-only local HTTP API** (`localhost:23119/api`, no key, no sync, `users/0`) and mature **local-Zotero MCP servers** now exist (e.g. `54yyyu/zotero-mcp`, `ZOTERO_LOCAL=true`). **Decisions taken**: pursue **MCP (live read) + an Acquire skill (durable ingest)** — the PRD's A+C, updated; **scope ingest to one Zotero collection** (`→ ai-wiki`) so the wider library never syncs; **commit no PDF binaries** (Zotero is the binary store; provenance via `zotero_item_key` + `doi`); source markdown **routes by type** (`raw/papers/`, `raw/articles/`, …). **Correction (mid-session, after a user screenshot):** `raw/` only *looks* markdown-only in the committed repo because `.gitignore` excludes all raw binaries (`raw/**/*.pdf` et al., local-only for copyright); the user's local working copy keeps the original PDFs co-located by type (e.g. `raw/articles/*.pdf`). **Touches**: 1 new thread page (zotero-stack-integratie) + index (Threads section) + this log. No concept/entity/source pages touched; no build executed yet. Reversible: delete the thread, revert index + log.
