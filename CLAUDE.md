...
7. Update `index.md` (catalog of pages, one-line summaries, organized by category). **Every page written or touched in this ingest needs its own bullet in its section** — not just the new source. The historically easy step to skip: when step 6 promotes a new entity (or concept), the entity gets its own page and gets mentioned inline inside the Source-section paragraph, but the standalone `## Entities` (or `## Concepts`) bullet is a separate edit that must also happen. This exact gap went undetected for months (28 entity pages existed with no index bullet before the 2026-07-04 bulk-refactor that fixed it) because nothing checked for it. Run `node scripts/lint-index-completeness.mjs` after any ingest that promotes a new page to verify — it's also surfaced automatically in the SessionStart snapshot's Lifecycle flags. **Sources** are ordered **by ingest batch, newest batch first** — not by a global date sort. Each ingest prepends its new sources as a *block* at the top of the Sources section, ordered reverse-chronologically (by the `YYYY-MM-DD` filename prefix) **within that block only**. Earlier blocks are never re-sorted or merged into the new one, so the section reads newest-batch-first while individual dates rise and fall across block boundaries — e.g. a 2026-09-10 source ingested after a 2026-09-14 one sits *above* it. This mirrors `wiki/log.md`, where one log entry corresponds to one block here, and it is what makes the two files diffable against each other. (Corrected 2026-09-15: this rule previously read *"ordered reverse-chronologically by the filename prefix"* globally, which the file has never actually satisfied — the practice has been batch-prepend since the reverse-chronological flip. The schema was describing an ordering nobody maintained; it now describes the one that is real. A global re-sort would destroy the batch↔log correspondence and is **not** wanted.) **Entities, Concepts, Syntheses, and Threads** stay flat-listed alphabetically (case-insensitive) — insert new bullets at their correct position rather than appending at the end.
...
- **Custom extensions** (in `extensions/`):
  ...
  - `inject-video-link.ts` — when a page's frontmatter `url:` points at YouTube, renders `▶ Watch on YouTube · <channel> · <MM:SS>` immediately after the H1 (channel from `author[0]`, duration from `length:`). Keyed on the URL, not `kind: video`, so podcast-kind pages with a YouTube URL get it too. Quartz renders no frontmatter `url:` on its own; this is why video source pages **do not need the URL repeated in body prose** — the frontmatter `url:` the video contract already mandates is sufficient.
...
| ----- | ------ | ------- |
| `SessionStart` | [`scripts/session-start.mjs`](scripts/session-start.mjs) | Outputs a short wiki snapshot (catalog counts, 5 most recent log entries — the *first* 5 since the 2026-05-12 reverse-chronological flip, `status: stale`, `confidence < 0.5`, and — since 2026-07-04 — pages missing from `index.md` per [`scripts/lint-index-completeness.mjs`](scripts/lint-index-completeness.mjs)) to stdout — Claude Code feeds it back as session context. Read-only. |
